...
function renderWireCatalog(m) {
  ...
  el.wireCalcNote.innerHTML = `单价规则：铝重 × 当前铝价 / 1,000,000 + 铜重 × 当前铜价 / 1,000,000 + 非铜/1000。铜价、铝价按元/吨输入，铝重 / 铜重 / 非铜按核算表原字段直接搬入。数据来源：${escapeHtml(sourceBooks || '导线联动核算表')}。当前 BOM 口径：${escapeHtml(bomLabel)}，用量来源：${escapeHtml(sourceSummary || wireUsageLabel(requestedUsage))}。`;
