(function () {
  ...
  let nativeBomStatus = '';
  let activeWorkbenchView = hasLocalStorage ? (window.localStorage.getItem(WORKBENCH_VIEW_STORAGE_KEY) || 'diff') : 'diff';
  ...
  function applyBomValidationViewState(viewState, preferredHarnessId = '') {
    ...
    semanticWorkbenchState = viewState.semanticWorkbench
      || buildSemanticWorkbenchStateFromComparisons(bomValidation?.comparisons || {}, {
  ...
  function sanitizeWorkbenchView(viewKey) {
    return WORKBENCH_VIEWS.some((view) => view.key === viewKey) ? viewKey : 'diff';
  }
  ...
  function buildWorkbenchStandardizedRow(comparison, group, sourceKey, item, row, rowIndex, partIndex = -1) {
    ...
    const normalizedName = buildNormalizedName(item.partName || item.partNumber);
    ...
    const supplier = displayList(item.suppliers);
    const series = detectWorkbenchSeries(item.partNumber, item.partName);
  ...
  function flattenWorkbenchRows(comparisons = {}) {
    const rawRows = [];
        ...
        (group.aligned || []).forEach((row, rowIndex) => {
          (comparison.versionOrder || []).forEach((sourceKey) => {
            ...
            if (directItem) {
              ...
              standardizedRows.push(buildWorkbenchStandardizedRow(comparison, group, sourceKey, directItem, row, rowIndex));
            }
            rowParts(row, sourceKey).forEach((partItem, partIndex) => {
              ...
              standardizedRows.push(buildWorkbenchStandardizedRow(comparison, group, sourceKey, partItem, row, rowIndex, partIndex));
            });
          });
        });
      });
  ...
  function inferWorkbenchDiffType(row, sourceKeys, groupKey) {
  ...
  function buildWorkbenchDiffRows(comparisons = {}) {
    const rows = [];
    Object.values(comparisons).forEach((comparison) => {
      ...
      (comparison.groups || []).forEach((group) => {
        (group.aligned || []).forEach((row, rowIndex) => {
          ...
          const diffType = inferWorkbenchDiffType(row, sourceKeys, group.key);
          const diffRow = {
            ...
            diffType,
            changeDetail: collapseText(row.relationSummary || row.relationTitle || row.relationDeltaText || ''),
  ...
  function buildWorkbenchDictionaries(standardizedRows = [], diffRows = []) {
    ...
    const seriesRows = uniqueOrdered(standardizedRows.map((row) => collapseText(row.series)).filter(Boolean)).map((series) => ({
      series,
    }));
  ...
  function buildSemanticWorkbenchStateFromComparisons(comparisons = {}, meta = {}) {
    const { rawRows, standardizedRows } = flattenWorkbenchRows(comparisons);
    const diffRows = buildWorkbenchDiffRows(comparisons);
    return {
      meta,
      rawRows,
      standardizedRows,
      diffRows,
      dictionaries: buildWorkbenchDictionaries(standardizedRows, diffRows),
    };
  }
