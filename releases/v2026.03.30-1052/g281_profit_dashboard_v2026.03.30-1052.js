...
const DEFAULT_STATE = { bom: 'freeze', metal: 'quote', connector: 'batch', labor: 'base', equipment: 'base', packaging: 'base', sales: 'quote', mix: 'quote', annualDrop: 'quote', oneTimeCustomer: 'quote', rebate: 'quote', vave: 'none' };
const USER_VERSION_STORAGE_KEY = 'g281.user.version.presets.v1';
const USER_VERSION_GROUPS = ['bom', 'metal', 'connector', 'labor', 'equipment', 'packaging', 'sales', 'mix', 'annualDrop', 'oneTimeCustomer', 'rebate', 'vave'];
...
function ensureDashboardUiScaffold() {
  ...
  if (main) {
    ...
    if (heroSection) {
      ...
      ['openBomValidationBtn', 'openCapitalValidationBtn', 'openLaborValidationBtn', 'openPackagingValidationBtn', 'openFactoryEfficiencyBtn'].forEach((id) => {
        const node = document.getElementById(id);
...
const VERSION_DISPLAY_ORDER = {
  ...
  mix: ['tt', 'fixed', 'quote'],
  annualDrop: ['tt', 'fixed', 'quote'],
  oneTimeCustomer: ['tt', 'fixed', 'quote'],
  rebate: ['tt', 'fixed', 'quote'],
  metal: ['tt', 'fixed', 'quote'],
};
const VERSION_TEMPLATE_GROUPS = new Set(['bom', 'metal', 'connector', 'labor', 'equipment', 'packaging', 'sales', 'mix', 'annualDrop', 'oneTimeCustomer', 'rebate']);
...
function annualDropVersionSnapshot(versionKey) {
  ...
  return {
    ...
    annualRate: coerceNumber(option.annualRate, 0),
...
function oneTimeCustomerVersionSnapshot(versionKey) {
  ...
  return {
    label: option.label || versionKey,
    amountTotal: coerceNumber(option.amountTotal, 0),
    sourceNote: option.sourceNote || option.note || '',
  };
}
...
function rebateVersionSnapshot(versionKey) {
  ...
  return {
    label: option.label || versionKey,
    amountPerSet: coerceNumber(option.amountPerSet, 0),
    sourceNote: option.sourceNote || option.note || '',
  };
}
...
function buildConnectorTemplateRows(activeKey, activeOption = {}) {
  ...
  if (savedRows.length) {
    return savedRows.map((row) => ({
      ...clonePlain(row, {}),
      currentStage: row.currentStage || currentOverrides[row.itemId] || activeKey,
      currentStageLabel: connectorTemplateStageDisplay(row.currentStage || currentOverrides[row.itemId] || activeKey, activeKey),
      recommendedStageLabel: connectorTemplateStageDisplay(row.recommendedStage || activeKey, activeKey),
    }));
  }

  const buckets = new Map();
  connectorItems.forEach((item) => {
    buckets.set(item.id, createConnectorTemplateSeedRow(item, activeKey, currentOverrides));
  });
  ...
  return connectorItems
    .map((item) => finalizeConnectorTemplateRow(buckets.get(item.id)))
    .filter((row) => row && (
      row.assemblyNo
      || row.partDetail
      || row.protocolPrice !== ''
...
function buildConnectorTemplatePayloadRows(context, rawInputs = {}) {
  ...
  return baseRows.map((row) => {
    ...
    const stageKey = connectorTemplateStageKeyFromInput(rawStage, row.currentStage || context?.activeKey || state.connector);
    return {
      ...clonePlain(row, {}),
      fieldKey,
      rawStageInput: rawStage,
      currentStage: stageKey,
      currentStageLabel: connectorTemplateStageDisplay(stageKey, context?.activeKey || state.connector),
    };
  });
}
...
function saveEditableBomWorkbookVersion(options = {}) {
  if (!BASE.versions?.bom) {
    throw new Error('BOM version store is not ready');
  }
...
function finalizeVersionTemplateContext(context, storedState = {}) {
  ...
  const fields = (context.fields || []).map((field, index) => ({
    ...field,
    address: field.address || templateFieldAddress(context.group, index),
  }));
...
function buildVersionTemplateContext(group) {
  ...
  const storedTemplateState = {
    ...
    workbookSnapshot: clonePlain(activeOption.templateWorkbookSnapshot, null),
    skipFieldOverlay: Boolean(activeOption.templateWorkbookSnapshot?.sheetOrder?.length > 1),
  };
  ...
  if (group === 'connector') {
    const rows = buildConnectorTemplateRows(activeKey, activeOption);
    ...
    const fields = rows.map((row, index) => ({
      ...
      hint: `可填写：${connectorTemplateStageHint(activeKey)}`,
      address: `M${index + 2}`,
      itemId: row.itemId,
    }));
    return finalizeVersionTemplateContext({
  ...
  if (group === 'annualDrop') {
    ...
    return finalizeVersionTemplateContext({
      ...
      eyebrow: '年降模板',
      title: '新增年降版本',
      subtitle: `参考当前 ${activeLabel}，录入 ASP 年度降幅后，系统会自动按年份递减售价。`,
      pasteHint: '支持粘贴单个降幅值，例如 0.02 表示每年下降 2%。',
      suggestedLabel: suggestNewVersionLabel(group),
      source: storedTemplateSource,
      note: storedTemplateNote,
      values: {
        annualRate: coerceNumber(snapshot.annualRate, 0),
      },
      fields: [
        { key: 'annualRate', label: '年度降幅', section: '商业条件', unit: 'x', step: '0.001', min: '0', aliases: ['年降', '年度降幅', 'ASP年降'] },
      ],
    }, storedTemplateState);
  }
  ...
  if (group === 'oneTimeCustomer') {
    ...
    return finalizeVersionTemplateContext({
      ...
      subtitle: `参考当前 ${activeLabel}，录入生命周期内由客户支付的一次性费用。`,
      pasteHint: '支持粘贴单个金额，单位为元。',
      suggestedLabel: suggestNewVersionLabel(group),
      source: storedTemplateSource,
      note: storedTemplateNote,
      values: {
        amountTotal: coerceNumber(snapshot.amountTotal, 0),
      },
      fields: [
        { key: 'amountTotal', label: '客户支付总额', section: '商业条件', unit: '元', step: '0.01', min: '0', aliases: ['一次性费用', '客户支付', '返补'] },
      ],
    }, storedTemplateState);
  }
  ...
  if (group === 'rebate') {
    ...
    return finalizeVersionTemplateContext({
      ...
      eyebrow: '返点模板',
      title: '新增返点版本',
      subtitle: `参考当前 ${activeLabel}，录入返给客户的单套返点金额。`,
      pasteHint: '支持粘贴单个金额，单位为元/套。',
      suggestedLabel: suggestNewVersionLabel(group),
      source: storedTemplateSource,
      note: storedTemplateNote,
      values: {
        amountPerSet: coerceNumber(snapshot.amountPerSet, 0),
      },
      fields: [
        { key: 'amountPerSet', label: '返点金额', section: '商业条件', unit: '元/套', step: '0.01', min: '0', aliases: ['返点', '返利', '返给客户'] },
      ],
    }, storedTemplateState);
  }
...
function syncVersionTemplatePanelMode(editorEnabled = false) {
  const panel = versionTemplatePanelElement();
  if (!panel) return;
  const isBomSheetMode = editorEnabled && ['bom', 'connector'].includes(versionTemplateDraft?.group);
  panel.classList.toggle('is-bom-sheet-mode', isBomSheetMode);
  panel.dataset.templateGroup = versionTemplateDraft?.group || '';
  if (el.versionTemplateQuickbar) {
    el.versionTemplateQuickbar.hidden = !isBomSheetMode;
  }
}
...
function runVersionTemplateQuickAction(action) {
  if (!versionTemplateEditor || !['bom', 'connector'].includes(versionTemplateDraft?.group)) {
...
function buildGenericVersionTemplateSheetSpec(context) {
  ...
  return {
    ...
    columnWidths: [52, 108, 240, 90, 140, 260],
    rowHeights: Array.from({ length: lastRow }, (_, index) => (index === 0 ? 38 : 30)),
    frozenRows: 1,
    styles: [
      { range: `A1:F${lastRow}`, border: { type: 'ALL', style: 'THIN', color: '#e2e8f0' } },
      { range: 'A1:F1', background: '#e3eaf2', fontWeight: 'bold', wrap: true, horizontalAlignment: 'center', verticalAlignment: 'middle' },
      ...
      { range: `C2:C${lastRow}`, wrap: true },
      { range: `D2:D${lastRow}`, horizontalAlignment: 'center', verticalAlignment: 'middle' },
      { range: `F2:F${lastRow}`, wrap: true },
    ],
    activeRange: context.fields[0]?.address || 'E2',
  };
}
...
function buildConnectorVersionTemplateSheetSpec(context) {
  ...
  return {
    ...
    sheetName: context.activeLabel || '模板',
    matrix: [
      ['NO.', '连接器', '总成号', '供应商', '协议价', '进度价', '初始报价', '状态', '已达成', '待确认', '开发中', '暂无回复', '执行档位', '散件明细', 'BOM定位'],
      ...rows.map((row, index) => ([
        index + 1,
        row.itemLabel,
        row.assemblyNo,
        ...
        row.noReplyCount ?? 0,
        templateSheetCellValue(context.rawInputs?.[connectorTemplateFieldKey(row.itemId)], row.currentStageLabel || ''),
        row.partDetail || '',
        row.bomLocation || '',
      ])),
    ],
    columnWidths: [56, 170, 220, 120, 92, 92, 92, 92, 68, 68, 68, 78, 108, 360, 180],
    rowHeights: Array.from({ length: lastRow }, (_, index) => (index === 0 ? 40 : 58)),
    frozenRows: 1,
    styles: [
      { range: `A1:O${lastRow}`, border: { type: 'ALL', style: 'THIN', color: '#dbe4ee' } },
      { range: 'A1:O1', background: '#e3eaf2', fontWeight: 'bold', wrap: true, horizontalAlignment: 'center', verticalAlignment: 'middle' },
      { range: `A2:A${lastRow}`, background: '#f8fafc', horizontalAlignment: 'center', verticalAlignment: 'middle' },
      { range: `B2:D${lastRow}`, background: '#fbfdff', wrap: true, verticalAlignment: 'middle' },
      { range: `E2:G${lastRow}`, horizontalAlignment: 'right', verticalAlignment: 'middle' },
      { range: `H2:L${lastRow}`, horizontalAlignment: 'center', verticalAlignment: 'middle' },
      { range: `M2:M${lastRow}`, background: '#fffef6', horizontalAlignment: 'center', verticalAlignment: 'middle' },
      { range: `N2:O${lastRow}`, wrap: true, verticalAlignment: 'top' },
    ],
    activeRange: context.fields[0]?.address || 'M2',
  };
}
...
function buildVersionTemplateSheetSpec(context) {
  if (context.workbookSnapshot) {
    return {
      ...
      activeRange: context.skipFieldOverlay ? 'A1' : (context.fields[0]?.address || (context.group === 'bom' ? 'F2' : 'E2')),
    };
  }
...
function openVersionTemplateModal(group) {
  ...
  setVersionTemplateWindowMaximized(['bom', 'connector'].includes(group));
...
function buildVersionTemplateSourceNote(group, context, source, note, values) {
  ...
  if (group === 'packaging') {
    return `${sourceText}；包装物流模板维护。单套包装物流 ${fmtMoney(values.packInner + values.packFreight + values.packWarehouse + values.packOther)} 元/套${noteText}`.trim();
  }
  if (group === 'sales') {
  ...
  if (group === 'annualDrop') {
    return `${sourceText}；年降模板维护。当前年度降幅 ${fmtPct(values.annualRate || 0)}${noteText}`.trim();
  }
  if (group === 'oneTimeCustomer') {
    return `${sourceText}；一次性费用模板维护。客户支付总额 ${fmtMoney(values.amountTotal || 0)} 元${noteText}`.trim();
  }
  if (group === 'rebate') {
    return `${sourceText}；返点模板维护。返点 ${fmtMoney(values.amountPerSet || 0)} 元/套${noteText}`.trim();
  }
...
function buildVersionTemplatePayload(group, context, values, source, note, rawInputs = {}, workbookSnapshot = null) {
  const sourceNote = buildVersionTemplateSourceNote(group, context, source, note, values);
  const templateState = {
    source,
    sourceNote,
    templateNote: note || '',
  ...
  if (group === 'connector') {
    ...
    const overrides = rows.reduce((acc, row) => {
      if (row.currentStage && row.currentStage !== context.activeKey) {
  ...
  if (group === 'annualDrop') {
    return {
      ...
      source,
      sourceNote,
      annualRate: Math.max(0, coerceNumber(values.annualRate, 0)),
    };
  }
  if (group === 'oneTimeCustomer') {
    return {
      ...
      source,
      sourceNote,
      amountTotal: Math.max(0, coerceNumber(values.amountTotal, 0)),
    };
  }
  if (group === 'rebate') {
    return {
      ...
      source,
      sourceNote,
      amountPerSet: Math.max(0, coerceNumber(values.amountPerSet, 0)),
    };
  }
  return {
    ...templateState,
...
function buildUserVersionOption(group, label, payload = {}) {
  ...
  if (group === 'connector') {
    return {
      ...
      sourceKey: payload.sourceKey || activeKey,
      overrides: sanitizeConnectorPricing(payload.overrides || connectorPricingState, activeKey),
  ...
  if (group === 'packaging') {
    return {
      ...
      sourceNote: payload.sourceNote || `来源：离线新增版本，复制自${currentLabel}，包装物流参数按当前页面快照保存。`,
    };
  }
  if (group === 'sales') {
    return {
  ...
  if (group === 'annualDrop') {
    return {
      ...baseOption,
      kind: payload.kind || activeOption.kind || 'custom',
      annualRate: payload.annualRate !== undefined ? Math.max(0, coerceNumber(payload.annualRate, 0)) : Math.max(0, coerceNumber(activeOption.annualRate, 0)),
      sourceNote: payload.sourceNote || `来源：离线新增版本，复制自${currentLabel}，当前年降 ${fmtPct(coerceNumber(activeOption.annualRate, 0))}。`,
    };
  }
  if (group === 'oneTimeCustomer') {
    return {
      ...baseOption,
      kind: payload.kind || activeOption.kind || 'custom',
      amountTotal: payload.amountTotal !== undefined ? Math.max(0, coerceNumber(payload.amountTotal, 0)) : Math.max(0, coerceNumber(activeOption.amountTotal, 0)),
      sourceNote: payload.sourceNote || `来源：离线新增版本，复制自${currentLabel}，客户一次性费用 ${fmtMoney(coerceNumber(activeOption.amountTotal, 0))} 元。`,
    };
  }
  if (group === 'rebate') {
    return {
      ...baseOption,
      kind: payload.kind || activeOption.kind || 'custom',
      amountPerSet: payload.amountPerSet !== undefined ? Math.max(0, coerceNumber(payload.amountPerSet, 0)) : Math.max(0, coerceNumber(activeOption.amountPerSet, 0)),
      sourceNote: payload.sourceNote || `来源：离线新增版本，复制自${currentLabel}，返点 ${fmtMoney(coerceNumber(activeOption.amountPerSet, 0))} 元/套。`,
    };
  }
  if (group === 'vave') {
    return {
...
function applyConnectorVersion(versionKey) {
  ...
  if (option?.userCreated) {
    connectorPricingState = sanitizeConnectorPricing(option.overrides || {}, state.connector);
    return;
  }
...
function applyVersionPreset(group, key) {
  ...
  if (group === 'annualDrop' || group === 'oneTimeCustomer' || group === 'rebate' || group === 'equipment' || group === 'vave') {
    dispatchDashboardVersionChange(group, key);
    return;
  }
}
...
function renderScenarioHistorySelect() {
  ...
  const selectedId = history.some((record) => record.id === lastSavedVersionId) ? lastSavedVersionId : '';
  const options = [
    '<option value="">当前场景（未保存）</option>',
    ...history.map((record) => {
      const label = `${record.name || record.scenarioName || record.id} · ${formatDateTime(record.createdAt)}`;
  ...
  el.scenarioHistorySelect.disabled = history.length === 0;
...
function renderEventTable(m){
  ...
  const rows=[
    ...
    ['年降',m.annualDrop?.label || '-',`${fmtPct(m.annualDrop?.annualRate || 0)} / ASP 逐年递减`],
    ['一次性费用',m.oneTimeCustomer?.label || '-',`${fmtMoney(m.oneTimeCustomer?.amountTotal || 0)} 元 / 生命周期`],
    ['返点',m.rebate?.label || '-',`${fmtMoney(m.rebate?.amountPerSet || 0)} 元 / 套`],
...
function bind(){
  ...
  if (el.scenarioHistorySelect) {
    el.scenarioHistorySelect.addEventListener('change', () => {
      const historyId = el.scenarioHistorySelect.value;
      if (!historyId) {
        lastSavedVersionId = '';
