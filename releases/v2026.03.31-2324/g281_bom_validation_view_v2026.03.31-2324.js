(function () {
  ...
  function renderCard(item, row, version, statusKey, groupKey, rowIndex, locked, options = {}) {
    ...
    const canToggleLock = Boolean(item) && Boolean(row?.lockable?.[version]) && locked && !options.disableDrag && !options.disableLockToggle;
    ...
    return `
      <article class="bom-card${canDrag ? '' : ' is-locked'}${cardClassName}" ${canDrag ? 'draggable="true"' : ''} title="${escapeHtml(titleText)}" data-item-key="${escapeHtml(item.itemKey)}" data-side="${escapeHtml(version)}" data-group-key="${escapeHtml(groupKey)}" data-index="${escapeHtml(String(rowIndex))}">
        ${canToggleLock ? `<button class="bom-card-lock" type="button" draggable="false" title="${escapeHtml(lockTitle)}" aria-label="${escapeHtml(lockTitle)}" data-lock-toggle="true" data-group-key="${escapeHtml(groupKey)}" data-side="${escapeHtml(version)}" data-index="${escapeHtml(String(rowIndex))}">${escapeHtml(lockLabel)}</button>` : ''}
  ...
  groups.addEventListener('click', (event) => {
    const lockButton = event.target.closest('[data-lock-toggle="true"]');
    if (!lockButton) return;
    event.preventDefault();
    event.stopPropagation();
    toggleRowLockState(
      select.value,
      lockButton.dataset.groupKey || '',
      lockButton.dataset.side || '',
  ...
  groups.addEventListener('dragstart', (event) => {
    if (event.target.closest('[data-lock-toggle="true"]')) {
      event.preventDefault();
      return;
    }
