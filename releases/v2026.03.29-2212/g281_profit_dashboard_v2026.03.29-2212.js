...
const VERSION_TEMPLATE_GROUPS = new Set(['bom', 'labor', 'equipment', 'packaging', 'sales', 'mix', 'annualDrop', 'oneTimeCustomer', 'rebate']);
...
function buildVersionTimelineEvents() {
  ...
  return timelineGroups.flatMap(([group, label]) => orderedVersionEntries(group, BASE.versions?.[group] || {}).map(([key, option]) => {
...
function syncVersionTemplatePanelMode(editorEnabled = false) {
  const panel = versionTemplatePanelElement();
  if (!panel) return;
  const isBomSheetMode = editorEnabled && versionTemplateDraft?.group === 'bom';
...
function runVersionTemplateQuickAction(action) {
  if (!versionTemplateEditor || versionTemplateDraft?.group !== 'bom') {
...
function buildVersionTemplateSheetSpec(context) {
  ...
  return context.group === 'bom'
    ? buildBomVersionTemplateSheetSpec(context)
    : buildGenericVersionTemplateSheetSpec(context);
}
...
function openVersionTemplateModal(group) {
  ...
  setVersionTemplateWindowMaximized(group === 'bom');
...
function buildUserVersionOption(group, label, payload = {}) {
  ...
  if (group === 'metal') {
    return {
      ...baseOption,
      copperPrice: Number(controls.copperPrice.value) || 0,
      aluminumPrice: Number(controls.aluminumPrice.value) || 0,
      manualManaged: true,
      seedSourceNote: `来源：离线新增版本，复制自${currentLabel}，铜铝基价按当前页面快照保存。`,
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，铜铝基价按当前页面快照保存。`,
    };
  }
  if (group === 'connector') {
    return {
      ...baseOption,
      factor: Number(activeOption.factor) || 1,
      sourceKey: activeKey,
      overrides: { ...connectorPricingState },
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，保留当前连接器覆盖项。`,
    };
  }
  if (group === 'labor') {
    return {
...
function collectVersionTimelineRows() {
  return ['bom', 'metal', 'connector', 'labor', 'equipment', 'packaging', 'annualDrop', 'oneTimeCustomer', 'rebate', 'vave']
...
function timelineColumnCount(eventCount, mountWidth) {
  ...
  if (mountWidth >= 1460) return Math.min(6, eventCount);
  if (mountWidth >= 1120) return Math.min(5, eventCount);
  if (mountWidth >= 840) return Math.min(4, eventCount);
  return Math.min(3, eventCount);
}
...
function renderVersionTimeline() {
  ...
  const mountWidth = Math.max(el.versionTimelineMount.clientWidth || 0, 720);
  const columns = timelineColumnCount(events.length, mountWidth);
  const rowHeight = columns <= 3 ? 112 : 104;
  const trackTop = 28;
  const cardTop = 44;
  const canvasHeight = Math.ceil(events.length / columns) * rowHeight + 20;
  const positionedEvents = events.map((event, index) => {
    const row = Math.floor(index / columns);
    const indexInRow = index % columns;
    const reverse = row % 2 === 1;
    const column = reverse ? columns - 1 - indexInRow : indexInRow;
    const left = columns === 1 ? 50 : 6 + (column * (88 / (columns - 1)));
    return {
      ...event,
      left,
      trackTop: row * rowHeight + trackTop,
      cardTop: row * rowHeight + cardTop,
    };
  });
  const latestEvent = positionedEvents[positionedEvents.length - 1];
  const groupCount = new Set(positionedEvents.map((event) => event.group)).size;
  const linePath = positionedEvents.map((event, index) => `${index === 0 ? 'M' : 'L'} ${event.left} ${event.trackTop}`).join(' ');
  el.versionTimelineMount.innerHTML = `
    <section class="timeline-snake" aria-label="\u7248\u672c\u53d1\u5e03\u65f6\u95f4\u7ebf">
      <div class="timeline-snake-head">
        <div class="timeline-snake-stats">
          <span class="timeline-stat">${positionedEvents.length} \u4e2a\u6210\u672c\u4e8b\u4ef6</span>
          <span class="timeline-stat">${groupCount} \u4e2a\u8981\u7d20\u8f68\u9053</span>
          <span class="timeline-stat">\u6700\u8fd1\u66f4\u65b0 ${escapeHtml(formatTimelineStamp(latestEvent.timestamp))}</span>
        </div>
      </div>
      <div class="timeline-canvas" style="height:${canvasHeight}px">
        <svg class="timeline-snake-svg" viewBox="0 0 100 ${canvasHeight}" preserveAspectRatio="none" aria-hidden="true">
          <path class="timeline-snake-line timeline-snake-line-shadow" d="${linePath}"></path>
        ...
        ${positionedEvents.map((event) => `
          ...
          <article class="timeline-event${event.active ? ' is-active' : ''}" style="left:${event.left}%; top:${event.cardTop}px" title="${escapeHtml(event.note || `${event.groupLabel} / ${event.label}`)}">
            <span class="timeline-event-group">${escapeHtml(event.groupLabel)}</span>
            <strong>${escapeHtml(event.label)}</strong>
            <span class="timeline-event-time">${escapeHtml(formatTimelineStamp(event.timestamp))}</span>
            ${event.active ? '<span class="timeline-event-flag">\u5f53\u524d\u4f7f\u7528</span>' : ''}
            ${event.note ? `<span class="timeline-event-note">${escapeHtml(summarizeTimelineNote(event.note))}</span>` : ''}
          </article>
        `).join('')}
      </div>
    </section>
