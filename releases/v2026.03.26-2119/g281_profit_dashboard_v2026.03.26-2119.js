...
function renderWireCatalog(m) {
  ...
  const rows = WIRE_MODELS.map((row) => {
    ...
    const aluminumCost = (aluminumWeight / 1000) * aluminumPrice;
    const copperCost = (copperWeight / 1000) * copperPrice;
  ...
  const totalUsage = rows.reduce((sum, row) => sum + row.usageValue, 0);
  const totalAmount = rows.reduce((sum, row) => sum + row.currentAmount, 0);
  ...
  const sourceSummary = Object.entries(sourceCounts)
    ...
    .join(' / ');
  const sourceBooks = [WIRE_CATALOG.meta?.quoteSourceWorkbook, WIRE_CATALOG.meta?.fixedSourceWorkbook]
    .filter(Boolean)
    .join(' / ');
  const summaryCards = [
    ['导线型号', `${WIRE_MODELS.length} 个`, '当前导线目录'],
    ['当前 BOM', bomLabel, `实取 ${sourceSummary || wireUsageLabel(requestedUsage)}`],
    ...
    ['当前导线金额', `${fmtMoney(totalAmount)} 元/套`, `当前用量 ${fmtNumber(totalUsage, 2)}`],
  ];
  ...
  el.wireCalcNote.innerHTML = `单价规则：铝重/1000 × 当前铝价 + 铜重/1000 × 当前铜价 + 非铜/1000。数据来源：${escapeHtml(sourceBooks || '导线联动核算表')}。当前 BOM 口径：${escapeHtml(bomLabel)}，用量来源：${escapeHtml(sourceSummary || wireUsageLabel(requestedUsage))}。`;
