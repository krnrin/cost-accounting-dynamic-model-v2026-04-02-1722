(function (global) {
  ...
  function extendLifecycleOutputs(runtime, result, versionKey) {
    const baselineKey = versionKey || 'quote';
    const stageCost = global.G281HarnessStageCostBuilder && typeof global.G281HarnessStageCostBuilder.build === 'function'
      ? global.G281HarnessStageCostBuilder.build(runtime, baselineKey)
      : { status: 'missing', rows: [] };
    const rollup = global.G281ProjectRollupBuilder && typeof global.G281ProjectRollupBuilder.build === 'function'
      ? global.G281ProjectRollupBuilder.build(stageCost.rows || [], runtime, { baselineKey: baselineKey })
      : null;
    const workflowBoard = global.G281WorkflowStageBoardBuilder && typeof global.G281WorkflowStageBoardBuilder.build === 'function'
      ? global.G281WorkflowStageBoardBuilder.build(runtime, { baselineKey: baselineKey })
      : null;
    const costExceptions = global.G281CostExceptionTrackerBuilder && typeof global.G281CostExceptionTrackerBuilder.build === 'function'
      ? global.G281CostExceptionTrackerBuilder.build(runtime, { baselineKey: baselineKey })
      : null;

    result.stageCosts = Object.assign({}, result.stageCosts, { [baselineKey]: stageCost });
    result.projectRollups = Object.assign({}, result.projectRollups, { [baselineKey]: rollup });
    result.workflowBoard = workflowBoard;
    result.costExceptions = costExceptions;
    if ((!result.harnessProfit || !safeArray(result.harnessProfit.harnesses).length) && global.G281HarnessProfit) {
      result.harnessProfit = global.G281HarnessProfit.buildHarnessProfitBreakdown(runtime, result, { versionKey: baselineKey });
    }
    return result;
  }

  function resolveComputeArgs(arg1, arg2, arg3) {
    if (arg1 && typeof arg1.getRuntime === 'function') {
      return {
        runtime: arg1.getRuntime() || global.G281_RUNTIME || {},
        draft: arg2 && arg3 ? arg2 : ((arg1.getHistory && arg1.getHistory()[0] && arg1.getHistory()[0].draft) || {}),
        state: arg3 || arg2 || {},
      };
    }
    return {
      runtime: arg1 || global.G281_RUNTIME || {},
      draft: arg2 || {},
      state: arg3 || {},
    };
  }
  ...
  function computeModelV2(runtime, draft, state) {
    ...
    return extendLifecycleOutputs(runtime, result, referenceFinancialKey || exactFinancialKey || 'quote');
  }

  function computeCompat(arg1, arg2, arg3) {
    const inputs = resolveComputeArgs(arg1, arg2, arg3);
    return computeModelV2(inputs.runtime, inputs.draft, inputs.state);
  }

  function computeLegacyCompat(arg1, arg2, arg3) {
    const inputs = resolveComputeArgs(arg1, arg2, arg3);
    return computeModel(inputs.runtime, inputs.draft, inputs.state);
  }
  ...
  global.G281Engine = {
    ...
    normalizeMix,
    summarizeBomChanges,
    computeModel: computeCompat,
    computeModelLegacy: computeLegacyCompat,
    resolvePureFinancialVersionKey,
    resolveReferenceFinancialVersionKey,
  };
  global.G281ComputeModel = {
    compute: computeCompat,
    computeLegacy: computeLegacyCompat,
  };
})(window);
