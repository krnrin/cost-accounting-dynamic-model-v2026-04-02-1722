(function (global) {
  ...
  function resolveStageMeta(runtimeInput, stageKey, options) {
    ...
    const requested = toText(stageKey || (options && options.stageKey), 'quote');
    return {
      stageKey: requested,
  ...
  function templateBuildCompat(runtimeInput, templateKey, baselineKey, options) {
    ...
    if (typeof adapter.buildPreview === 'function') {
      ...
      return adapter.buildPreview({
        ...
        templateKey,
        baselineKey,
        stageKey: toText(options && options.stageKey, baselineKey || 'quote'),
  ...
  function build(runtimeInput, stageKey, options) {
    ...
    const stageMeta = resolveStageMeta(runtimeInput, stageKey || safeOptions.stageKey || safeOptions.baselineKey, safeOptions);
    const primaryKey = stageMeta.financialKey || stageMeta.stageKey;
    const comparisonKey = stageMeta.comparisonKey || (primaryKey === 'quote' ? 'fixed' : 'quote');
    ...
    const projectMetrics = buildProjectMetricEntries(primaryRollup, comparisonRollup);
    const projectDiff = {
      stageMeta: {
        stageKey: stageMeta.stageKey,
        financialKey: primaryKey,
      ...
      from: safeObject(comparisonRollup.perSet),
      to: safeObject(primaryRollup.perSet),
      delta: PER_SET_KEYS.reduce((accumulator, key) => {
        accumulator[key] = projectMetrics[key].delta;
        return accumulator;
      }, {}),
      metrics: projectMetrics,
      revenuePerSetDelta: projectMetrics.revenue.delta,
      costPerSetDelta: projectMetrics.cost.delta,
      profitPerSetDelta: projectMetrics.profit.delta,
      marginDelta: projectMetrics.margin.delta,
    };
    ...
    const templateDiffs = safeArray(primaryTemplate.fields).map((field) => {
    ...
    const recoveryImpact = buildRecoveryImpact(bomChanges);
    ...
    return {
      ...
      summary: {
        ...
        templateDiffCount: templateDiffs.length,
        obsoleteValue: recoveryImpact.obsoleteValue,
        projectCostPerSetDelta: projectDiff.costPerSetDelta,
        projectProfitPerSetDelta: projectDiff.profitPerSetDelta,
      },
    };
  }
