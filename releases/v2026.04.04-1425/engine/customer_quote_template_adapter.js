(function (global) {
  ...
  function templateMatches(template, templateKey) {
    const effectiveKey = toText(templateKey, DEFAULT_TEMPLATE_KEY);
    const aliases = [
    ...
    return aliases.includes(effectiveKey);
  }
  ...
  function templateSpec(templateKey, config) {
    const requestedKey = toText(templateKey, DEFAULT_TEMPLATE_KEY);
    const templates = config && config.customerTemplates ? config.customerTemplates : {};
    if (Array.isArray(templates)) {
      const match = templates.find((template) => templateMatches(template, requestedKey))
        || templates.find((template) => templateMatches(template, DEFAULT_TEMPLATE_KEY))
        || templates.find((template) => toText(template && template.channel, '') === 'customer');
      if (match && Array.isArray(match.cells) && match.cells.length) {
        return {
          key: toText(match.key || match.id, requestedKey),
          label: toText(match.name || match.label, 'Customer quote template'),
          cells: match.cells,
        };
      }
    } else {
      const direct = templates[requestedKey] || templates[DEFAULT_TEMPLATE_KEY];
      if (direct && Array.isArray(direct.cells) && direct.cells.length) return direct;
    }
  ...
  function build(runtimeInput, templateKey, stageKey, options) {
    ...
    const context = {
      projectCode,
      projectName,
      customer: toText(config && config.customer, runtime && runtime.master && runtime.master.customer),
