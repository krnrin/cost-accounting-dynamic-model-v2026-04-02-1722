...
function timelineColumnCount(eventCount, mountWidth) {
  ...
  if (mountWidth >= 1500) return Math.min(10, eventCount);
  if (mountWidth >= 1220) return Math.min(9, eventCount);
  if (mountWidth >= 880) return Math.min(7, eventCount);
  if (mountWidth >= 640) return Math.min(6, eventCount);
  return Math.min(5, eventCount);
}
...
function renderVersionTimeline() {
  ...
  const rowHeight = columns >= 7 ? 52 : 58;
  const trackTop = 16;
  const labelTop = 25;
  const canvasHeight = Math.ceil(events.length / columns) * rowHeight + 6;
  ...
  el.versionTimelineMount.innerHTML = `
    <section class="timeline-snake" aria-label="\u7248\u672c\u53d1\u5e03\u65f6\u95f4\u7ebf">
      <div class="timeline-snake-head">
        <span class="timeline-snake-meta">${positionedEvents.length} \u4e2a\u4e8b\u4ef6 \u00b7 \u66f4\u65b0\u81f3 ${escapeHtml(formatTimelineDay(latestEvent.timestamp))}</span>
      </div>
      <div class="timeline-canvas" style="height:${canvasHeight}px">
        ...
        ${positionedEvents.map((event) => `
          ...
          <span class="timeline-event${event.active ? ' is-active' : ''}" style="left:${event.left}%; top:${event.labelTop}px" title="${escapeHtml(`${event.groupLabel} / ${event.label} / ${formatTimelineStamp(event.timestamp)}${event.note ? ` / ${event.note}` : ''}`)}">${escapeHtml(compactTimelineEventLabel(event))}</span>
        `).join('')}
      </div>
    </section>
  `;
