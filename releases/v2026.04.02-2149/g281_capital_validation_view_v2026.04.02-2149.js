(function () {
  ...
  const EDIT_STORAGE_KEY = 'g281.capitalManualEdit.v1';
  ...
  let manualEditState = hasLocalStorage ? safeParse(window.localStorage.getItem(EDIT_STORAGE_KEY), {}) : {};
  let dragState = null;
  ...
  function saveEditState() {
    if (!hasLocalStorage) return;
    window.localStorage.setItem(EDIT_STORAGE_KEY, JSON.stringify(manualEditState));
  }
  ...
  function getScopeEditState(scopeId) {
    if (!manualEditState[scopeId]) {
      manualEditState[scopeId] = {};
    }
    return manualEditState[scopeId];
  }

  function getItemEdit(scopeId, side, itemKey) {
    const scope = getScopeEditState(scopeId);
    const lane = scope[side] || {};
    return lane[itemKey] || null;
  }

  function setItemEdit(scopeId, side, itemKey, patch) {
    const scope = getScopeEditState(scopeId);
  ...
  function clearScopeEdits(scopeId) {
    delete manualEditState[scopeId];
    saveEditState();
  }
  ...
  function applyItemEdit(scopeId, side, item) {
    if (!item || !item.itemKey) return item;
    const edit = getItemEdit(scopeId, side, item.itemKey);
    if (!edit) return item;
    return {
      ...item,
      ...edit,
  ...
  function renderCard(item, peerItem, side, statusKey, comparison, groupKey, rowIndex, locked) {
    ...
    const editFields = editMode ? `
        <div class="bom-inline-field" style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 6px;">
          <input class="capital-edit-input" data-field="newQty" data-side="${escapeHtml(side)}" data-item-key="${escapeHtml(item.itemKey)}" type="text" value="${escapeHtml(item.newQty ?? '')}" placeholder="数量">
          <input class="capital-edit-input" data-field="unitPrice" data-side="${escapeHtml(side)}" data-item-key="${escapeHtml(item.itemKey)}" type="text" value="${escapeHtml(item.unitPrice ?? '')}" placeholder="单价">
          <input class="capital-edit-input" data-field="newAmount" data-side="${escapeHtml(side)}" data-item-key="${escapeHtml(item.itemKey)}" type="text" value="${escapeHtml(item.newAmount ?? '')}" placeholder="金额">
          <input class="capital-edit-input" data-field="note" data-side="${escapeHtml(side)}" data-item-key="${escapeHtml(item.itemKey)}" type="text" value="${escapeHtml(item.note ?? '')}" placeholder="备注">
        </div>
      ` : '';
  ...
  function renderLaneCell(groupKey, side, item, peerItem, statusKey, index, locked, comparison) {
    const scopedItem = applyItemEdit(select.value, side, item);
    const peerSide = side === 'quote' ? 'fixed' : 'quote';
    const scopedPeer = applyItemEdit(select.value, peerSide, peerItem);
    ...
    return `
      <div class="bom-align-cell${locked ? ' is-locked' : ''}" ${isDropZone ? 'data-drop-zone="true"' : ''} data-group-key="${escapeHtml(groupKey)}" data-side="${escapeHtml(side)}" data-drop-index="${escapeHtml(String(index))}">
        ${renderCard(scopedItem, scopedPeer, side, statusKey, comparison, groupKey, index, locked)}
      </div>
    `;
  }
  ...
  function renderScope(scopeId) {
    ...
    if (!comparison) {
      summary.innerHTML = '';
      groups.innerHTML = '<div class="bom-compare-empty-state">未找到该模块的资源投入管理数据。</div>';
      return;
    }
    ...
    const summaryItems = [
      ...
      `当前激活对比列 <strong>定点 / 报价</strong>`,
      `<button type="button" data-capital-action="toggle-edit">${editMode ? '完成编辑' : '编辑模式'}</button>`,
      '<button type="button" data-capital-action="clear-edit">清除本模块编辑</button>',
    ];
  ...
  openBtn.textContent = `资源投入管理 (${capitalValidation.scopeOrder.length})`;
    openBtn.textContent = `\u8d44\u6e90\u6295\u5165\u7ba1\u7406 (${capitalValidation.scopeOrder.length})`;
  ...
  summary.addEventListener('click', (event) => {
    const action = event.target?.dataset?.capitalAction;
    if (!action) return;
  ...
  groups.addEventListener('change', (event) => {
    ...
    if (!input || !input.classList || !input.classList.contains('capital-edit-input')) return;
    const scopeId = select.value;
    const itemKey = input.dataset.itemKey;
    const side = input.dataset.side;
    const field = input.dataset.field;
    if (!scopeId || !itemKey || !side || !field) return;

    const patch = field === 'note'
      ? { [field]: input.value }
      : { [field]: toNumberOrRaw(input.value) };
    setItemEdit(scopeId, side, itemKey, patch);
    renderScope(scopeId);
  });
