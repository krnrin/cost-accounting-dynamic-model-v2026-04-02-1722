(function (root, factory) {
  ...
  const GENERIC_BOM_TEMPLATE_SHEETS = [
    { key: 'changeHistory', label: '变更履历', matchers: ['变更履历'], keepRows: [1], preserveRowsUntil: 1 },
    { key: 'assemblyParts', label: '总成散件清单', matchers: ['总成散件清单'], keepRows: [1], preserveRowsUntil: 1 },
    { key: 'secondaryMaterials', label: '二次物料明细', matchers: ['二次物料明细'], keepRows: [1, 2], preserveRowsUntil: 2 },
    { key: 'kskHarnessBom', label: 'KSK线束BOM明细', matchers: ['KSK线束BOM明细'], keepRows: [1], preserveRowsUntil: 1 },
    { key: 'harnessSheet', label: '线束页-01', sourceKind: 'firstHarness', keepRows: [1, 4], preserveRowsUntil: 4 },
  ];
  ...
  function normalizeRowNumbers(rows, fallbackMaxRow = 1) {
    ...
    return [ensurePositiveInteger(fallbackMaxRow, 1)];
  }
  ...
  function listAvailableVersionKeys(source) {
    const payload = ensureObject(source);
    const ordered = ensureArray(payload.versionOrder).filter((key) => payload.versions?.[key]);
    return ordered.length ? ordered : Object.keys(ensureObject(payload.versions));
  }

  function resolveVersionPayload(source, versionKey) {
    const payload = ensureObject(source);
    const normalizedKey = ensureString(versionKey, '');
    if (normalizedKey && payload.versions?.[normalizedKey]) {
      return {
        versionKey: normalizedKey,
        version: ensureObject(payload.versions[normalizedKey]),
      };
    }
    const fallbackKey = listAvailableVersionKeys(payload)[0] || '';
    return {
      versionKey: fallbackKey,
      version: ensureObject(payload.versions?.[fallbackKey]),
    };
  }
  ...
  function filterCellsByRows(cells, keepRows) {
    const keepRowSet = new Set(normalizeRowNumbers(keepRows));
    return ensureArray(cells)
      .filter((cell) => keepRowSet.has(Number(cell?.row)))
      .map((cell) => clonePlain(cell, {}));
  }

  function filterRowDimensionsByLimit(rowDimensions, maxRow) {
    const preserveLimit = ensurePositiveInteger(maxRow, 1);
    return ensureArray(rowDimensions)
      .filter((row) => {
        const rowIndex = Number(row?.row);
        return Number.isFinite(rowIndex) && rowIndex >= 1 && rowIndex <= preserveLimit;
      })
      .map((row) => clonePlain(row, {}));
  }

  function filterHiddenRowsByLimit(hiddenRows, maxRow) {
    const preserveLimit = ensurePositiveInteger(maxRow, 1);
    return ensureArray(hiddenRows).filter((row) => {
      const rowIndex = Number(row);
      return Number.isFinite(rowIndex) && rowIndex >= 1 && rowIndex <= preserveLimit;
    });
  }

  function filterMergedRangesByLimit(mergedRanges, maxRow) {
    const preserveLimit = ensurePositiveInteger(maxRow, 1) - 1;
    return ensureArray(mergedRanges).filter((rangeRef) => {
      const decoded = decodeRange(rangeRef);
      return Boolean(decoded && decoded.s.r <= preserveLimit && decoded.e.r <= preserveLimit);
    });
  }

  function buildHeaderOnlySheet(sourceSheet, options) {
    const settings = ensureObject(options);
    const keepRows = normalizeRowNumbers(settings.keepRows, settings.preserveRowsUntil || 1);
    const preserveRowsUntil = Math.max(
      ensurePositiveInteger(settings.preserveRowsUntil, 0),
      keepRows[keepRows.length - 1] || 1,
    );
    return {
      workbookSheetIndex: Number.isFinite(Number(sourceSheet?.workbookSheetIndex))
        ? Number(sourceSheet.workbookSheetIndex)
        : null,
      sheetName: ensureString(settings.label, ensureString(sourceSheet?.sheetName, 'Sheet1')),
      sheetState: ensureString(sourceSheet?.sheetState, 'visible'),
      isHidden: false,
      sheetOrderKey: ensureString(settings.sheetOrderKey, ensureString(settings.label, ensureString(sourceSheet?.sheetName, 'Sheet1'))),
      dimensionRef: ensureString(sourceSheet?.dimensionRef, null),
      maxRow: Math.max(Number(sourceSheet?.maxRow) || 0, DEFAULT_SNAPSHOT_ROW_COUNT),
      maxColumn: Math.max(Number(sourceSheet?.maxColumn) || 0, DEFAULT_SNAPSHOT_COLUMN_COUNT),
      sheetFormat: clonePlain(ensureObject(sourceSheet?.sheetFormat), {}),
      sheetView: clonePlain(ensureObject(sourceSheet?.sheetView), {}),
      freezePane: ensureString(sourceSheet?.freezePane, null),
      mergedRanges: filterMergedRangesByLimit(sourceSheet?.mergedRanges, preserveRowsUntil),
      rowDimensions: filterRowDimensionsByLimit(sourceSheet?.rowDimensions, preserveRowsUntil),
      columnDimensions: clonePlain(ensureArray(sourceSheet?.columnDimensions), []),
      hiddenRows: filterHiddenRowsByLimit(sourceSheet?.hiddenRows, preserveRowsUntil),
      hiddenColumns: clonePlain(ensureArray(sourceSheet?.hiddenColumns), []),
      cells: filterCellsByRows(sourceSheet?.cells, keepRows),
      styleTable: clonePlain(ensureObject(sourceSheet?.styleTable), {}),
    };
  }

  function findFirstHarnessSheet(version, normalizedSheets, indexedTemplates) {
    const excludedNames = new Set(
      Object.values(ensureObject(indexedTemplates))
        .filter(Boolean)
        .map((sheet) => sheet.sheetName),
    );
    const sheetByName = normalizedSheets.reduce((acc, sheet) => {
      acc[sheet.sheetName] = sheet;
      return acc;
    }, {});
    const sourceOrder = ensureArray(version.sheetOrder);
    for (let index = 0; index < sourceOrder.length; index += 1) {
      const sheetName = sourceOrder[index];
      if (excludedNames.has(sheetName)) continue;
      if (sheetByName[sheetName]) {
        return sheetByName[sheetName];
      }
    }
    return normalizedSheets.find((sheet) => !excludedNames.has(sheet.sheetName)) || null;
  }
  ...
  function buildGenericBomTemplateContext(options) {
    const normalizedOptions = ensureObject(options);
    const source = normalizeRuntimeWorkbookSource(inferSource(normalizedOptions.source));
    const projectId = ensureString(normalizedOptions.projectId, ensureString(source.contract?.projectId, 'generic-bom-template'));
    const resolvedVersion = resolveVersionPayload(source, ensureString(normalizedOptions.versionKey, ''));
    const version = ensureObject(resolvedVersion.version);
    if (!Object.keys(version).length) {
      throw new Error('[G281BomTemplateRuntime] Workbook copy version not found for generic BOM template');
    }

    const styleTable = clonePlain(ensureObject(version.styleTable), {});
    const normalizedSheets = ensureArray(version.sheets).map((sheet) => normalizeSheet(sheet, styleTable));
    const sharedSheetConfig = normalizeTargetSheetConfig(
      GENERIC_BOM_TEMPLATE_SHEETS
        .filter((definition) => definition.sourceKind !== 'firstHarness')
        .map((definition) => ({
          key: definition.key,
    ...
    const indexedTemplates = indexTemplateSheets(normalizedSheets, sharedSheetConfig);
    const harnessSourceSheet = findFirstHarnessSheet(version, normalizedSheets, indexedTemplates);
    if (!harnessSourceSheet) {
      throw new Error('[G281BomTemplateRuntime] No harness sheet found for generic BOM template');
    }

    const templateSheets = GENERIC_BOM_TEMPLATE_SHEETS.map((definition) => {
      const sourceSheet = definition.sourceKind === 'firstHarness'
        ? harnessSourceSheet
        : (indexedTemplates[definition.key] || normalizedSheets.find((sheet) => sheetMatches(sheet.sheetName, definition.matchers)));
      if (!sourceSheet) {
        throw new Error(`[G281BomTemplateRuntime] Missing template sheet source: ${definition.label}`);
      }
      return {
        key: definition.key,
        sheet: buildHeaderOnlySheet(sourceSheet, definition),
      };
    });

    const workbookName = ensureString(normalizedOptions.workbookName, '通用BOM模板');
    const sourceFileName = ensureString(normalizedOptions.sourceFileName, `${workbookName}.xlsx`);
    const workbook = {
      workbookName,
      sourceFileName,
      sourcePath: ensureString(normalizedOptions.sourcePath, null),
      versionKey: ensureString(normalizedOptions.templateVersionKey, 'generic-bom-template'),
      versionLabel: ensureString(normalizedOptions.templateVersionLabel, workbookName),
      sheetOrder: templateSheets.map((entry) => entry.sheet.sheetName),
      hiddenSheets: [],
      styleTable,
      sheets: templateSheets.map((entry) => entry.sheet),
    };

    return {
      kind: 'g281-generic-bom-template-context',
      projectId,
      versionKey: ensureString(resolvedVersion.versionKey, ensureString(normalizedOptions.versionKey, '')),
      versionLabel: workbook.versionLabel,
      sourceVersionKey: ensureString(resolvedVersion.versionKey, ''),
      sourceVersionLabel: ensureString(version.versionLabel, ensureString(resolvedVersion.versionKey, '')),
      generatedAt: source.generatedAt,
      sourceDirectory: source.sourceDirectory,
      contract: clonePlain(source.contract, {}),
      templateSheetConfig: clonePlain(GENERIC_BOM_TEMPLATE_SHEETS, []),
      workbook,
      workbookName,
      sourceFileName,
      templates: templateSheets.reduce((acc, entry) => {
        acc[entry.key] = entry.sheet;
        return acc;
      }, {}),
    };
  }
  ...
  function buildWorkbookSnapshotFromSeed(workbookSeed, options) {
    return convertIntermediateWorkbookToUniverSnapshot(ensureObject(workbookSeed), options);
  }
  ...
  function buildVersionTemplateContextWithUniver(options) {
    ...
    return {
      ...context,
      workbookName: ensureString(context?.workbook?.workbookName, ''),
      sourceFileName: ensureString(context?.workbook?.sourceFileName, ''),
  ...
  function buildGenericBomTemplateContextWithUniver(options) {
    const context = buildGenericBomTemplateContext(options);
    return {
      ...context,
      workbookName: ensureString(context?.workbook?.workbookName, ''),
      sourceFileName: ensureString(context?.workbook?.sourceFileName, ''),
  ...
  return {
    ...
    GENERIC_BOM_TEMPLATE_SHEETS: clonePlain(GENERIC_BOM_TEMPLATE_SHEETS, []),
    normalizeRuntimeWorkbookSource,
    buildVersionTemplateContext,
    buildVersionTemplateContextWithUniver,
    buildGenericBomTemplateContext,
    buildGenericBomTemplateContextWithUniver,
    buildVersionWorkbookSnapshot,
    buildWorkbookSnapshotFromSeed,
