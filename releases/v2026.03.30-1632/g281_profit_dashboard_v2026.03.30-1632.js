...
function buildVersionTemplateContext(group) {
  ...
  const hasStoredWorkbookSnapshot = Boolean(activeOption.templateWorkbookSnapshot?.sheetOrder?.length);
  const storedTemplateState = {
    ...
    skipFieldOverlay: hasStoredWorkbookSnapshot
      ? Boolean(activeOption.templateWorkbookSnapshot?.sheetOrder?.length > 1)
      : null,
  };
