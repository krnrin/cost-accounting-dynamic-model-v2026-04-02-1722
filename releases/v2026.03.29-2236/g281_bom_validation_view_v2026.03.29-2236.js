(function () {
  ...
  async function refreshNativeBomStatus(message = '') {
    ...
    nativeBomStatus = message || `原生 BOM 库已入库 ${nativeBomVersionCount} 个版本，支持 ${window.XLSX ? 'XLSX / JSON' : 'JSON'} 导入导出`;
  }
  ...
  function rowMatchStateDynamic(row, sourceKeys) {
    ...
    if (sourceCount >= 3) return 'full_match';
  ...
  function syncHarnessSelect(preferredHarnessId = '') {
    ...
    openBtn.textContent = `BOM 绠＄悊 (${harnessOrder.length})`;
  ...
  async function buildRuntimeBomValidationView() {
    ...
    const activeVersionEntry = bridge?.listBomVersions?.().find((entry) => entry.key === activeBomKey) || null;
    ...
    if (!resolvedSource?.workbookSource) {
      return buildSeedViewState(`褰撳墠 BOM 鐗堟湰 ${activeOption.label || activeBomKey} 鏈寘鍚彲鐢ㄧ殑绾挎潫鏄庣粏宸ヤ綔琛紝BOM 绠＄悊鏆傛椂缁х画鎸夊唴缃?${shortLabel(activeWorkbookVersionKey)} 瀵规瘮瑙嗗浘鏄剧ず銆?`);
    }

    const parsed = parseWorkbookHarnesses(resolvedSource.workbookSource);
    if (!parsed.harnessOrder.length) {
      return buildSeedViewState(`褰撳墠 BOM 鐗堟湰 ${activeOption.label || activeBomKey} 鏈В鏋愬埌绾挎潫椤碉紝BOM 绠＄悊鏆傛椂缁х画鎸夊唴缃?${shortLabel(activeWorkbookVersionKey)} 瀵规瘮瑙嗗浘鏄剧ず銆?`);
    }
    ...
    return {
      ...
      baseVersion,
      syncHint: `BOM 绠＄悊宸插悓姝ュ埌褰撳墠宸︿晶 BOM 鐗堟湰锛?${activeOption.label || activeBomKey}銆?`,
    };
  }
  ...
  async function syncBomValidationView(options = {}) {
    ...
    try {
      ...
      applyBomValidationViewState(
        buildSeedViewState(`BOM 绠＄悊鍚屾澶辫触锛?{error.message}`),
        preferredHarnessId,
      );
    }
  }
