...
function renderConnectorProtocolOverview() {
  ...
  const note = '初始化规则：同一聚合连接器下全部映射项已达成时切协议价；若已有部分达成则切进度价；若仍全部未达成，则先按样品价。下方明细只用于判断整套连接器的谈价状态，不单独计价。';
...
function renderConnectorPricing(m){
  ...
  el.connectorPriceTable.innerHTML = (m.connectorItems || []).map((item) => {
    ...
    const stageLabel = item.selectionLabel || connectorStageMeta(stageKey).label;
    const itemDeltaClass = item.deltaCost > 0 ? 'delta-up' : item.deltaCost < 0 ? 'delta-down' : 'delta-flat';
    const recommendedStage = recommendedConnectorStage(protocolSummary);
    const recommendedLabel = recommendedStage ? connectorStageMeta(recommendedStage).label : '跟随默认';
    const protocolStatusText = protocolSummary ? protocolCountsInline(protocolSummary) : '未配置';
    const protocolDetailRows = renderProtocolDetailRows(item.id);
    const functionLines = protocolFunctionLines(item.id);
    const supplierLabel = protocolSupplierLabel(item.id, item.supplier || '-');
    const sourceCount = Number(protocolSummary?.sourceCount) || protocolRowsForItem(item.id).length || 0;
    const connectorTitle = functionLines.length
      ? functionLines.map((line, index) => index === 0
        ? `<strong title="${escapeHtml(line)}">${escapeHtml(line)}</strong>`
        : `<div class="subtle connector-title-line" title="${escapeHtml(line)}">${escapeHtml(line)}</div>`).join('')
      : `<strong>${escapeHtml(item.name || item.code || '-')}</strong>`;
    const connectorMeta = [
      item.quantity ? `Qty ${item.quantity}` : '',
      sourceCount ? `Excel ${sourceCount}` : '',
    ].filter(Boolean).join(' / ');
    const buttons = [
      ...
      ...connectorSelectableStageKeys.map((versionKey) => `<button class="connector-option${item.overrideKey === versionKey ? ' active' : ''}" type="button" data-connector-id="${item.id}" data-connector-stage="${versionKey}">${connectorStageMeta(versionKey).label}</button>`),
    ].join('');
    ...
    return `<tr>
      <td>
        <div class="connector-title">
          ${connectorTitle}
          ${connectorMeta ? `<div class="subtle">${escapeHtml(connectorMeta)}</div>` : ''}
        </div>
      </td>
      <td>
        <div>${escapeHtml(supplierLabel || '-')}</div>
        <div class="subtle">连接器占比 ${fmtPct(item.share)}</div>
        <div class="connector-protocol-summary">
          ${protocolDetailRows}
        </div>
      </td>
      <td>
      ...
      <td>
        <div class="connector-stage">
          ...
          ${item.selectionKey === 'progress' ? `<div class="subtle">${protocolProgressText(item)}</div>` : ''}
