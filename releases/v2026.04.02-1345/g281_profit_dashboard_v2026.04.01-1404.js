      ...
      <div class="workflow-detail-grid">
        ...
        <div class="workflow-detail-section">
          ...
          <div class="workflow-detail-kv">
            <div><span>责任角色</span><strong>${escapeHtml(record.currentOwnerRoles.join(' / ') || '-')}</strong></div>
            <div><span>影响模块</span><strong>${escapeHtml(record.impactedModules.join(' / ') || '项目初始')}</strong></div>
...
function renderWorkflowWorkspace(model) {
  ...
  const heroKpis = selectedRecord
    ? [
      { label: '当前责任', value: selectedRecord.currentOwnerRoles.join(' / ') || '-', meta: '按当前阶段显示可编辑角色' },
