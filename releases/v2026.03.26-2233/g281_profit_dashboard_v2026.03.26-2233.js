...
const USER_VERSION_GROUPS = ['bom', 'metal', 'connector', 'labor', 'equipment', 'packaging', 'sales', 'mix', 'vave'];
...
function buildUserVersionOption(group, label) {
  ...
  const baseOption = {
    ...
    userCreated: true,
    createdAt,
    note: `离线新增版本，复制自${currentLabel}，保存在当前浏览器。`,
  };
  ...
  if (group === 'bom') {
    ...
    return {
      ...baseOption,
      factor: Number(snapshot?.materialFactor) || Number(activeOption.factor) || 1,
      draft: currentBomDraftSnapshot(),
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，BOM参数按当前页面快照保存。`,
  ...
  if (group === 'labor') {
    return {
      ...baseOption,
      ...currentLaborDraftSnapshot(),
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，工时与费率按当前页面快照保存。`,
    };
  }
  if (group === 'equipment') {
    return {
      ...baseOption,
      factor: Number(activeOption.factor) || 1,
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，当前设备投资系数 ${fmtNumber(Number(activeOption.factor) || 1, 3)}。`,
    };
  }
  if (group === 'packaging') {
    return {
      ...baseOption,
      ...currentPackagingDraftSnapshot(),
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，包装物流参数按当前页面快照保存。`,
    };
  }
  if (group === 'sales') {
    return {
      ...baseOption,
      volumes: currentSalesDraftSnapshot(),
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，销量预测按当前页面快照保存。`,
    };
  }
  if (group === 'mix') {
    return {
      ...baseOption,
      values: currentMixDraftSnapshot(),
      sourceNote: `来源：离线新增版本，复制自${currentLabel}，配置比例按当前页面快照保存。`,
    };
  }
  if (group === 'vave') {
    return {
