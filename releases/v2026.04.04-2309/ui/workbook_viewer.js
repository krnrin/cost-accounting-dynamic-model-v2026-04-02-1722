(function (global) {
  ...
  async function resolveBomSource(versionKey) {
    ...
    if (nativeVersionId && global.G281FactorVersionRepo?.create) {
      try {
        const snapshot = await global.G281FactorVersionRepo
          .create({ projectCode: bridge?.getProjectCode?.() || 'default-project' })
          .getSnapshotByVersionId(nativeVersionId, 'bom');
        if (snapshot) {
          return {
            datasetKey: 'bom',
            versionKey: normalizedKey,
        ...
        console.warn('[G281WorkbookViewer] Failed to resolve factor-version workbook snapshot', error);
      }
    }
    ...
    const runtimeWorkbookSource = getRuntime().bomWorkbookCopies?.versions?.[workbookVersionKey] || null;
    if (runtimeWorkbookSource) {
      const snapshot = toSnapshotFromWorkbookSource(runtimeWorkbookSource, {
        workbookName: toText(
          option?.workbook,
          toText(
            versionEntry?.workbook,
            toText(runtimeWorkbookSource.sourceFileName, toText(versionEntry?.label, normalizedKey)),
          ),
        ),
      });
      if (snapshot) {
        return {
          ...
          sourceLabel: toText(runtimeWorkbookSource.sourceFileName, toText(versionEntry?.source, '')),
          workbookName: toText(
            option?.workbook,
            toText(versionEntry?.workbook, toText(runtimeWorkbookSource.sourceFileName, normalizedKey)),
          ),
          workbookVersionKey,
          userCreated: Boolean(option?.userCreated),
          canSave: true,
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `
      ...
      <section class="bom-modal-panel version-template-panel workbook-editor-panel is-bom-sheet-mode" role="dialog" aria-modal="true" aria-labelledby="workbookViewerTitle">
        <div class="bom-modal-head version-template-head workbook-viewer-head">
          <h3 id="workbookViewerTitle" class="workbook-viewer-title-sr">Excel 整表编辑</h3>
          <div class="workbook-viewer-head-bar">
            <select id="workbookViewerVersion" class="workbook-viewer-version-select" aria-label="版本"></select>
        ...
        <div class="version-template-quickbar workbook-viewer-quickbar">
          <div class="workbook-viewer-quick-group workbook-viewer-save-group">
            <button class="button primary version-template-quickbtn" id="workbookViewerSaveBtn" type="button">保存当前版</button>
            <button class="button ghost version-template-quickbtn" id="workbookViewerSaveAsBtn" type="button">另存新版本</button>
          </div>
          ...
          <div class="workbook-viewer-quick-group workbook-viewer-zoom-group">
            ...
            <span class="workbook-viewer-zoom-value" id="workbookViewerZoomValue">100%</span>
    ...
    refs.zoomValue = modal.querySelector('#workbookViewerZoomValue');
  ...
  function updateStatusMeta() {
    ...
    const activeSheetName = toText(
      viewerState.editor?.getActiveSheet?.()?.getSheetName?.(),
      viewerState.sheetName,
    );
    if (activeSheetName) {
      viewerState.sheetName = activeSheetName;
      if (refs.sheet && refs.sheet.value !== activeSheetName) {
        refs.sheet.value = activeSheetName;
      }
    }
    ...
    if (refs.zoomValue) {
      refs.zoomValue.textContent = zoomLabel;
    }
    if (refs.sheetMeta) {
  ...
  function renderVersionSelect() {
    ...
    refs.version.disabled = !entries.length;
  ...
  async function saveWorkbook(mode) {
    ...
    const label = mode === 'update-current'
      ? toText(viewerState.sourceMeta?.versionLabel, viewerState.versionKey)
      : toText(refs.name?.value, '');
    const source = toText(
      refs.source?.value,
      viewerState.sourceMeta?.sourceLabel || viewerState.sourceMeta?.workbookName || '',
    );

    try {
      const result = await bridge.saveEditableBomWorkbookVersion({
        mode,
  ...
  global.G281WorkbookViewer = publicApi;
  global.G281UI = global.G281UI || {};
  global.G281UI.WorkbookViewer = publicApi;
