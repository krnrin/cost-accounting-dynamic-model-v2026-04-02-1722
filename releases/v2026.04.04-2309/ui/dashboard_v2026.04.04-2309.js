...
let versionTemplateModalController = null;
let factoryEfficiencyModalController = null;
let versionTemplateLastFocused = null;
function openManagedModal(modal, controller, options = {}) {
  if (!modal) return;
...
function closeManagedModal(modal, controller, options = {}) {
  if (!modal) return;
...
function setFactoryEfficiencyModalOpen(open, model = null) {
  ...
  if (open) {
    ...
    openManagedModal(el.factoryEfficiencyModal, factoryEfficiencyModalController, {
      bodyClass: 'bom-modal-open',
      openClass: 'is-window-open',
    });
  ...
  closeManagedModal(el.factoryEfficiencyModal, factoryEfficiencyModalController, {
    bodyClass: 'bom-modal-open',
    openClass: 'is-window-open',
  });
...
function loadStoredWorkspacePage() {
  try {
    return window.localStorage?.getItem(WORKSPACE_PAGE_STORAGE_KEY) === 'data' ? 'data' : 'profit';
  } catch (error) {
    return 'profit';
  }
}
...
function bind(){
  ...
  if (window.G281UI && window.G281UI.ModalController) {
    if (el.versionTemplateModal) {
      versionTemplateModalController = new window.G281UI.ModalController({
        modal: el.versionTemplateModal,
        backdropSelector: '[data-version-template-close], .bom-modal-backdrop',
        bodyClass: 'bom-modal-open',
        openClass: 'is-window-open',
        onRequestClose: closeVersionTemplateModal,
      });
      versionTemplateModalController.init();
    }
    if (el.factoryEfficiencyModal) {
      factoryEfficiencyModalController = new window.G281UI.ModalController({
        modal: el.factoryEfficiencyModal,
        backdropSelector: '[data-factory-efficiency-close], .bom-modal-backdrop',
        bodyClass: 'bom-modal-open',
        openClass: 'is-window-open',
        onRequestClose: () => setFactoryEfficiencyModalOpen(false),
      });
      factoryEfficiencyModalController.init();
    }
  }
