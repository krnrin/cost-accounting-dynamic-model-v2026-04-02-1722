(function (global) {
  ...
  function buildWorkflowThresholdEntries(config) {
    return safeArray(config && config.workflowNodes).map((node, index) => ({
      id: toText(node && node.id, `workflow-node-${index + 1}`),
      page: toText(node && node.page, ''),
      stageIds: safeArray(node && node.stageIds).map((stageId) => resolveStageKey(stageId, config, stageId)).filter(Boolean),
      thresholds: safeObject(node && node.thresholds),
    }));
  }

  function thresholdEntriesForNode(node, thresholdEntries) {
    const aliases = new Set(
      []
        .concat([toText(node && node.key, ''), toText(node && node.artifactType, '')])
        .concat(safeArray(node && node.aliases))
        .map((value) => toText(value, ''))
        .filter(Boolean)
    );
    return safeArray(thresholdEntries).filter((entry) => {
      if (entry.page && entry.page === toText(node && node.page, '')) return true;
      return safeArray(entry.stageIds).some((stageId) => aliases.has(stageId));
    });
  }

  function buildThresholdBreaches(workflowBoard, thresholdEntries) {
    return safeArray(workflowBoard && workflowBoard.nodes).map((node) => {
      const matchedEntries = thresholdEntriesForNode(node, thresholdEntries);
      const thresholds = matchedEntries.length ? safeObject(matchedEntries[0].thresholds) : {};
      const breaches = [];
      const completionRateThreshold = Number(thresholds.completionRate);
      const blockedCountThreshold = Number(thresholds.blockedCount);
      const exceptionCountThreshold = Number(thresholds.exceptionCount);

      if (Number.isFinite(completionRateThreshold) && numberOr(node && node.completionRate, 0) < completionRateThreshold) {
        breaches.push({
          metric: 'completionRate',
          threshold: completionRateThreshold,
          actual: numberOr(node && node.completionRate, 0),
          direction: 'below',
        });
      }
      if (Number.isFinite(blockedCountThreshold) && numberOr(node && node.blockedCount, 0) > blockedCountThreshold) {
        breaches.push({
          metric: 'blockedCount',
          threshold: blockedCountThreshold,
          actual: numberOr(node && node.blockedCount, 0),
          direction: 'above',
        });
      }
      if (Number.isFinite(exceptionCountThreshold) && numberOr(node && node.exceptionCount, 0) > exceptionCountThreshold) {
        breaches.push({
          metric: 'exceptionCount',
          threshold: exceptionCountThreshold,
          actual: numberOr(node && node.exceptionCount, 0),
          direction: 'above',
        });
      }
      ...
      return {
        nodeKey: toText(node && node.key, ''),
        label: toText(node && node.label, toText(node && node.key, '')),
        page: toText(node && node.page, ''),
        thresholds,
        metrics: {
          completionRate: numberOr(node && node.completionRate, 0),
          blockedCount: numberOr(node && node.blockedCount, 0),
          exceptionCount: numberOr(node && node.exceptionCount, 0),
        },
        severity: breaches.some((breach) => breach.metric === 'blockedCount' || breach.metric === 'exceptionCount') ? 'high' : 'medium',
        breachCount: breaches.length,
        breaches,
        detail: breaches.map((breach) => `${breach.metric} ${breach.direction} threshold (${breach.actual} vs ${breach.threshold})`).join(' | '),
        thresholdSourceIds: matchedEntries.map((entry) => entry.id),
      };
    }).filter(Boolean);
  }
  ...
  function buildFocusQueue(sections, thresholdBreaches) {
    const queue = [];

    safeArray(thresholdBreaches).forEach((breach) => {
      queue.push({
        category: 'Workflow threshold',
        label: breach.label,
        detail: breach.detail,
        severity: breach.severity,
        priority: 95,
        magnitude: breach.breachCount,
      });
    });
  ...
  function buildOverview(sections, thresholdBreaches) {
    return [
      {
        key: 'protocol',
        label: '协议价异常',
  ...
  function build(runtimeInput, options) {
    ...
    const workflowBoard = safeOptions.workflowBoard && safeOptions.workflowBoard.nodes
      ? safeOptions.workflowBoard
      : (global.G281WorkflowStageBoardBuilder && typeof global.G281WorkflowStageBoardBuilder.build === 'function'
        ? global.G281WorkflowStageBoardBuilder.build(runtimeInput, { baselineKey, config })
        : { status: 'missing', baselineKey, nodes: [] });
    const workflowThresholds = buildWorkflowThresholdEntries(config);
    const thresholdBreaches = buildThresholdBreaches(workflowBoard, workflowThresholds);
    ...
    return {
      ...
      focusQueue: buildFocusQueue(sections, thresholdBreaches),
      overview: buildOverview(sections, thresholdBreaches),
      summaries: {
        workflowThresholds: {
          breachCount: thresholdBreaches.length,
        },
