(function (global) {
  ...
  function resolveRuntimeInput(runtimeInput) {
  ...
  function looksLikeStateSnapshot(value) {
    if (!value || Array.isArray(value) || typeof value !== 'object') return false;
    const stateKeys = ['bom', 'metal', 'connector', 'labor', 'equipment', 'packaging', 'sales', 'mix', 'annualDrop', 'oneTimeCustomer', 'rebate', 'vave'];
    return stateKeys.some((key) => Object.prototype.hasOwnProperty.call(value, key));
  }

  function resolveComputeArgs(runtimeInput, draft, state) {
    if (state !== undefined) {
      return {
        runtime: resolveRuntimeInput(runtimeInput),
        draft: draft || {},
        state: state || {},
        runtimeInput,
      };
    }
    if (looksLikeStateSnapshot(draft)) {
      return {
        runtime: resolveRuntimeInput(runtimeInput),
        draft: {},
        state: draft || {},
  ...
  function lifecycleStageKeys() {
    return ['quote', 'fixed', 'change', 'annualDrop'];
  }

  function normalizeLifecycleStageMeta(stageMeta, stageKey) {
    const candidate = clonePlain(stageMeta, null);
    const normalized = candidate || {
      stageKey,
      baselineKey: stageKey,
      lifecycleStageKey: stageKey,
    };
    if (!normalized.stageKey) normalized.stageKey = stageKey;
    if (!normalized.baselineKey) normalized.baselineKey = stageKey;
    if (!normalized.lifecycleStageKey) normalized.lifecycleStageKey = normalized.stageKey || stageKey;
    return normalized;
  }

  function buildByStageMap(stageCosts, projectRollups) {
    const byStage = {};
    lifecycleStageKeys().forEach((stageKey) => {
      const stageCost = stageCosts[stageKey] || null;
      const projectRollup = projectRollups[stageKey] || null;
      const stageMetaCandidate = stageCost && stageCost.stageMeta
        ? stageCost.stageMeta
        : projectRollup && projectRollup.stageMeta
          ? projectRollup.stageMeta
          : null;
      const stageMeta = normalizeLifecycleStageMeta(stageMetaCandidate, stageKey);
      byStage[stageKey] = {
        stageKey,
        lifecycleStageKey: stageMeta.lifecycleStageKey || stageKey,
        stageMeta,
        stageCost,
        projectRollup,
      };
  ...
  function extendLifecycleOutputs(runtimeInput, model) {
    const warnings = model && model.financialContext && Array.isArray(model.financialContext.warnings)
      ? model.financialContext.warnings
      : [];
    const baselineKey = (model && model.financialContext && (model.financialContext.referenceKey || model.financialContext.exactKey))
      || resolveReferenceFinancialVersionKey(model && model.stateSnapshot)
      || 'quote';
    const stageCosts = {};
    const projectRollups = {};

    lifecycleStageKeys().forEach((stageKey) => {
      const stageOptions = { stageKey, baselineKey: stageKey, lifecycleStageKey: stageKey };
      try {
        if (global.G281HarnessStageCostBuilder && typeof global.G281HarnessStageCostBuilder.build === 'function') {
          stageCosts[stageKey] = global.G281HarnessStageCostBuilder.build(runtimeInput, stageKey, stageOptions);
        }
      } catch (error) {
        warnings.push(`stageCosts.${stageKey} build failed: ${error && error.message ? error.message : String(error)}`);
        stageCosts[stageKey] = { status: 'error', stageKey, rows: [], summary: {}, matrix: null };
      }

      try {
        if (global.G281ProjectRollupBuilder && typeof global.G281ProjectRollupBuilder.build === 'function') {
          projectRollups[stageKey] = stageCosts[stageKey] && Array.isArray(stageCosts[stageKey].rows)
            ? global.G281ProjectRollupBuilder.build(stageCosts[stageKey].rows, runtimeInput, stageOptions)
            : global.G281ProjectRollupBuilder.build(runtimeInput, null, stageOptions);
        }
      } catch (error) {
        warnings.push(`projectRollups.${stageKey} build failed: ${error && error.message ? error.message : String(error)}`);
        projectRollups[stageKey] = { status: 'error', baselineKey: stageKey, perSet: {}, totals: {}, lifecycle: [] };
      }
    });

    const byStage = buildByStageMap(stageCosts, projectRollups);

    let workflowBoard = null;
    try {
      if (global.G281WorkflowStageBoardBuilder && typeof global.G281WorkflowStageBoardBuilder.build === 'function') {
        workflowBoard = global.G281WorkflowStageBoardBuilder.build(runtimeInput, {
          baselineKey,
        });
      }
    } catch (error) {
      warnings.push(`workflowBoard build failed: ${error && error.message ? error.message : String(error)}`);
      workflowBoard = null;
    }

    let costExceptions = null;
    try {
      if (global.G281CostExceptionTrackerBuilder && typeof global.G281CostExceptionTrackerBuilder.build === 'function') {
        costExceptions = global.G281CostExceptionTrackerBuilder.build(runtimeInput, {
          baselineKey,
          workflowBoard,
        });
      }
    } catch (error) {
      warnings.push(`costExceptions build failed: ${error && error.message ? error.message : String(error)}`);
      costExceptions = null;
    }

    model.stageCosts = stageCosts;
    model.projectRollups = projectRollups;
    model.byStage = byStage;
    model.workflowBoard = workflowBoard;
    model.costExceptions = costExceptions;
    return model;
  }

  function computeModelV2(runtime, draft, state, runtimeInput) {
    ...
    return extendLifecycleOutputs(runtimeInput || runtime, result);
  }

  function computeModelCompat(runtimeInput, draft, state) {
    const resolved = resolveComputeArgs(runtimeInput, draft, state);
    return computeModelV2(resolved.runtime, resolved.draft, resolved.state, resolved.runtimeInput);
  }
  ...
  global.G281Engine = {
    ...
    normalizeMix,
    summarizeBomChanges,
    computeModel: computeModelCompat,
    computeModelV2,
    computeModelLegacy: computeModel,
    resolvePureFinancialVersionKey,
    resolveReferenceFinancialVersionKey,
  };
  global.G281ComputeModel = {
    compute: computeModelCompat,
    computeV2: computeModelCompat,
    computeLegacy: computeModel,
  };
})(window);
