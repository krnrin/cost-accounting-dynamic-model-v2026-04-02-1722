(function (global) {
  ...
  function normalizeConnectorPricing(rawPricing, items, versions) {
    ...
    return Object.entries(rawPricing || {}).reduce((acc, [itemId, versionKey]) => {
      if (validIds.has(itemId) && versions[versionKey]) {
  ...
  function buildConnectorScenario(base, versions, defaultKey, draftPricing) {
    ...
    if (!items.length || !baseCostPerSet) {
      return {
        ...
        overrideCount: 0,
        followCount: 0,
        stageCounts: { batch: 0, protocol: 0, sample: 0 },
      };
    }

    const stageCounts = { batch: 0, protocol: 0, sample: 0 };
    let totalBaseCost = 0;
    ...
    const connectorItems = items.map((item) => {
      ...
      const selectionKey = connectorVersionKey(versions, explicitKey, defaultKey);
      const selection = versions[selectionKey] || versions[defaultKey];
  ...
  function computeModel(runtime, draft, state) {
    ...
    const labor = BASE.versions.labor[currentState.labor];
    const equip = BASE.versions.equipment[currentState.equipment];
    const pack = BASE.versions.packaging[currentState.packaging];
    const vave = BASE.versions.vave[currentState.vave];
    const connectorScenario = buildConnectorScenario(BASE, BASE.versions.connector, currentState.connector, d.connectorPricing);
    ...
    const directLabor = d.directHours * d.directRate * labor.factor;
    const manufacturing = d.manufacturingHours * d.manufacturingRate * labor.factor;
    const packaging = (d.packInner + d.packFreight + d.packWarehouse + d.packOther) * pack.factor;
    const equipment = BASE.baseEquipmentPerSet * equip.factor;
    ...
    const capitalTotal = BASE.capital.equipment + BASE.capital.tooling + BASE.capital.fixtures + BASE.capital.rnd;
    const capitalPerSet = capitalTotal / totalVolume;
    ...
    const baseCapitalTotal = capitalTotal;
