(function () {
  ...
  function resolveDocumentStatusMap() {
    const map = {
      SAME: { label: '\u4e00\u81f4', className: 'same' },
      ADDED: { label: '\u65b0\u589e', className: 'added' },
      REMOVED: { label: '\u53d6\u6d88', className: 'removed' },
      SUBSTITUTED: { label: '\u5df2\u5224\u5b9a\u66ff\u4ee3', className: 'substituted' },
      QTY_CHANGED: { label: '\u6570\u91cf\u53d8\u66f4', className: 'qty-changed' },
      WIRE_SIZE_CHANGED: { label: '\u7ebf\u5f84\u53d8\u66f4', className: 'wire-size-changed' },
      POLE_COUNT_CHANGED: { label: '\u5b54\u4f4d\u53d8\u66f4', className: 'pole-count-changed' },
      LENGTH_CHANGED: { label: '\u957f\u5ea6\u53d8\u66f4', className: 'length-changed' },
      NEED_REVIEW: { label: '\u9700\u590d\u6838', className: 'review-needed' },
      ASSEMBLY_TO_PARTS: { label: '\u603b\u6210\u6620\u5c04', className: 'assembly-map' },
      assembly_bundle: { label: '\u603b\u6210\u6620\u5c04', className: 'assembly-map' },
      assembly_part: { label: '\u6563\u4ef6\u5c55\u5f00', className: 'assembly-part' },
      full_match: { label: '\u4e00\u81f4', className: 'same' },
      partial_match: { label: '\u5355\u4fa7\u5360\u4f4d', className: 'review-needed' },
      replacement: { label: '\u5df2\u5224\u5b9a\u66ff\u4ee3', className: 'substituted' },
    };
    versionOrder.forEach((version) => {
      map[`${version}_only`] = {
        label: versionOrder.length === 2
          ? (version === baseVersion ? '\u53d6\u6d88' : '\u65b0\u589e')
          : `${shortLabel(version)}\u72ec\u6709`,
        className: version === baseVersion ? 'removed' : 'added',
      };
    });
    return map;
  }
  ...
  function rebuildStatusMap() {
    statusMap = resolveDocumentStatusMap();
  }

  function resolveStatusMeta(statusKey, version = '') {
    return statusMap[statusKey]
      || (version ? statusMap[`${version}_only`] : null)
      || statusMap.NEED_REVIEW
      || { label: '\u9700\u590d\u6838', className: 'review-needed' };
  }
  ...
  function buildDiffRowKey(harnessId, groupKey, rowIndex) {
    return `${normalizeKey(harnessId)}::${normalizeKey(groupKey)}::${Number(rowIndex)}`;
  }

  function documentStatusFromDiffRow(row) {
    const diffType = collapseText(row?.diffType);
    switch (diffType) {
      case 'SAME':
      case 'SAME_SPEC_RENAMED':
        return 'SAME';
      case 'ADDED':
      case 'REMOVED':
      case 'SUBSTITUTED':
      case 'QTY_CHANGED':
      case 'WIRE_SIZE_CHANGED':
      case 'POLE_COUNT_CHANGED':
      case 'LENGTH_CHANGED':
      case 'ASSEMBLY_TO_PARTS':
        return diffType;
      case 'POSSIBLE_SUBSTITUTE':
      case 'SPEC_CHANGED':
      case 'SHIELDING_CHANGED':
      case 'WATERPROOF_CHANGED':
        return 'NEED_REVIEW';
      default:
        return collapseText(row?.riskLevel) === 'High' ? 'NEED_REVIEW' : 'NEED_REVIEW';
    }
  }

  function needsReviewForDiffRow(row) {
    const statusKey = collapseText(row?.statusKey) || documentStatusFromDiffRow(row);
    if (statusKey === 'NEED_REVIEW') return true;
    if (['WIRE_SIZE_CHANGED', 'POLE_COUNT_CHANGED', 'LENGTH_CHANGED'].includes(statusKey)) return true;
    if (['SAME', 'ADDED', 'REMOVED', 'SUBSTITUTED', 'QTY_CHANGED', 'ASSEMBLY_TO_PARTS'].includes(statusKey)) return false;
    return collapseText(row?.riskLevel) === 'High';
  }

  function createDocumentStatusSummary() {
    return {
      total: 0,
      diffOnly: 0,
      needReviewTotal: 0,
      counts: DOCUMENT_STATUS_ORDER.reduce((acc, key) => {
        acc[key] = 0;
        return acc;
      }, {}),
    };
  ...
  function appendDocumentStatusSummary(summary, row) {
    const next = summary || createDocumentStatusSummary();
    const statusKey = collapseText(row?.statusKey) || documentStatusFromDiffRow(row);
    if (!Object.prototype.hasOwnProperty.call(next.counts, statusKey)) {
      next.counts[statusKey] = 0;
    }
    next.total += 1;
    next.counts[statusKey] += 1;
    if (statusKey !== 'SAME') next.diffOnly += 1;
    if (row?.needReview ?? needsReviewForDiffRow(row)) next.needReviewTotal += 1;
    return next;
  }

  function summarizeDocumentStatusRows(rows = []) {
    return (Array.isArray(rows) ? rows : []).reduce((summary, row) => appendDocumentStatusSummary(summary, row), createDocumentStatusSummary());
  }

  function indexWorkbenchDiffRows(diffRows = []) {
    const diffRowIndex = {};
    const harnessStatusSummaryIndex = {};
    const groupStatusSummaryIndex = {};
    (Array.isArray(diffRows) ? diffRows : []).forEach((row) => {
      const harnessKey = normalizeKey(row.harnessNo || row.harnessId);
      const groupSummaryKey = `${harnessKey}::${normalizeKey(row.groupKey)}`;
      diffRowIndex[buildDiffRowKey(row.harnessNo || row.harnessId, row.groupKey, row.rowIndex)] = row;
      if (!harnessStatusSummaryIndex[harnessKey]) {
        harnessStatusSummaryIndex[harnessKey] = createDocumentStatusSummary();
      }
      if (!groupStatusSummaryIndex[groupSummaryKey]) {
        groupStatusSummaryIndex[groupSummaryKey] = createDocumentStatusSummary();
      }
      appendDocumentStatusSummary(harnessStatusSummaryIndex[harnessKey], row);
      appendDocumentStatusSummary(groupStatusSummaryIndex[groupSummaryKey], row);
    });
    return {
      diffRowIndex,
      harnessStatusSummaryIndex,
      groupStatusSummaryIndex,
    };
  }

  function diffRowForGroupRow(harnessId, groupKey, rowIndex) {
    return semanticWorkbenchState?.diffRowIndex?.[buildDiffRowKey(harnessId, groupKey, rowIndex)] || null;
  }

  function harnessStatusSummaryFor(harnessId) {
    return semanticWorkbenchState?.harnessStatusSummaryIndex?.[normalizeKey(harnessId)] || createDocumentStatusSummary();
  }

  function groupStatusSummaryFor(harnessId, groupKey) {
    return semanticWorkbenchState?.groupStatusSummaryIndex?.[`${normalizeKey(harnessId)}::${normalizeKey(groupKey)}`] || createDocumentStatusSummary();
  }
  ...
  function inferWorkbenchDiffType(row, sourceKeys, groupKey, dictionaryState = null) {
    ...
    if (matchWorkbenchSubstituteRule(baseItem, activeItem, dictionaryState)) return 'SUBSTITUTED';
    const baseQty = numericValue(baseItem.quantity);
    const activeQty = numericValue(activeItem.quantity);
  ...
  function inferWorkbenchRiskLevel(diffType) {
    ...
    if (['POSSIBLE_SUBSTITUTE', 'SUBSTITUTED', 'ASSEMBLY_TO_PARTS', 'ADDED', 'REMOVED'].includes(diffType)) return 'Medium';
    return 'Low';
  }
    ...
    Object.values(comparisons).forEach((comparison) => {
      ...
      (comparison.groups || []).forEach((group) => {
        (group.aligned || []).forEach((row, rowIndex) => {
          ...
          const riskLevel = inferWorkbenchRiskLevel(diffType);
          const statusKey = documentStatusFromDiffRow({ diffType, riskLevel });
          ...
          const diffRow = {
            ...
            needReview: needsReviewForDiffRow({ diffType, riskLevel, statusKey }),
            changeDetail: [
  ...
  function buildWorkbenchDictionaries(standardizedRows = [], diffRows = []) {
    ...
    const substituteRows = diffRows
      .filter((row) => ['SUBSTITUTED', 'POSSIBLE_SUBSTITUTE', 'SPEC_CHANGED', 'SAME_SPEC_RENAMED'].includes(row.diffType))
      .map((row) => ({
  ...
  function buildSemanticWorkbenchStateFromComparisons(comparisons = {}, meta = {}, dictionaryState = null) {
    ...
    const indexedDiffRows = indexWorkbenchDiffRows(diffRows);
    ...
    return {
      ...
      standardizedRows,
      diffRows,
      diffRowIndex: indexedDiffRows.diffRowIndex,
      harnessStatusSummaryIndex: indexedDiffRows.harnessStatusSummaryIndex,
      groupStatusSummaryIndex: indexedDiffRows.groupStatusSummaryIndex,
  ...
  function isPendingReviewRow(row) {
    ...
    if (!row?.needReview) return false;
  ...
  function filterWorkbenchDiffRows(rows = []) {
    ...
    return rows.filter((row) => row.statusKey !== 'SAME').filter((row) => {
      ...
      if (filters.pendingOnly && (!row.needReview || !isPendingReviewRow(row))) return false;
      return true;
    });
  }
  ...
  function classifyDiffExportSheet(row) {
    ...
    if (/端子|TERMINAL/i.test(namePool)) return 'TERMINAL';
    if (row.businessCategory === 'CONNECTOR') return 'CONNECTOR';
    if (row.businessCategory === 'CABLE') return 'CABLE';
    if (row.businessCategory === 'PROTECTION') return 'PROTECTION';
    if (row.businessCategory === 'FASTENER') return 'FASTENER';
    return '';
  }
  ...
  function buildDiffExportRow(row) {
    return {
      ...
      Need_Review: Boolean(row.needReview),
  ...
  function exportCurrentWorkbenchDiff(harnessId) {
    ...
    const allDiffRows = rawRows.map((row) => ({
      ...row,
      statusKey: row.statusKey || documentStatusFromDiffRow(row),
      statusLabel: row.statusLabel || resolveStatusMeta(documentStatusFromDiffRow(row)).label,
      needReview: row.needReview ?? needsReviewForDiffRow(row),
    }));
    const diffOnlyRows = allDiffRows.filter((row) => row.statusKey !== 'SAME');
    const needReviewRows = allDiffRows.filter((row) => row.needReview);
    ...
    const summary = summarizeDocumentStatusRows(allDiffRows);
    const summaryRows = [
      ...
      { Metric: 'AllDiff_Total', Value: allDiffRows.length },
      { Metric: 'DiffOnly_Total', Value: diffOnlyRows.length },
      { Metric: 'NeedReview_Total', Value: needReviewRows.length },
      { Metric: 'Same', Value: summary.counts.SAME || 0 },
      { Metric: 'Added', Value: summary.counts.ADDED || 0 },
      { Metric: 'Removed', Value: summary.counts.REMOVED || 0 },
      { Metric: 'Substituted', Value: summary.counts.SUBSTITUTED || 0 },
      { Metric: 'Qty_Changed', Value: summary.counts.QTY_CHANGED || 0 },
      { Metric: 'Wire_Size_Changed', Value: summary.counts.WIRE_SIZE_CHANGED || 0 },
      { Metric: 'Pole_Count_Changed', Value: summary.counts.POLE_COUNT_CHANGED || 0 },
      { Metric: 'Length_Changed', Value: summary.counts.LENGTH_CHANGED || 0 },
      { Metric: 'Need_Review_Status', Value: summary.counts.NEED_REVIEW || 0 },
      { Metric: 'Assembly_To_Parts', Value: summary.counts.ASSEMBLY_TO_PARTS || 0 },
      { Metric: 'High_Risk', Value: diffOnlyRows.filter((row) => row.riskLevel === 'High').length },
      { Metric: 'Pending_Review', Value: needReviewRows.filter((row) => isPendingReviewRow(row)).length },
      { Metric: 'Exported_At', Value: new Date().toISOString() },
    ];
    const allExportRows = allDiffRows.map(buildDiffExportRow);
    const diffOnlyExportRows = diffOnlyRows.map(buildDiffExportRow);
    const needReviewExportRows = needReviewRows.map(buildDiffExportRow);
    const groupedRows = diffOnlyRows.reduce((acc, row) => {
      const sheetName = classifyDiffExportSheet(row);
      if (!sheetName) return acc;
      if (!acc[sheetName]) acc[sheetName] = [];
      acc[sheetName].push(buildDiffExportRow(row));
      return acc;
    }, {});
    ...
    if (window.XLSX?.utils?.book_new) {
      ...
      window.XLSX.utils.book_append_sheet(workbook, window.XLSX.utils.json_to_sheet(allExportRows), 'AllDiff');
      window.XLSX.utils.book_append_sheet(workbook, window.XLSX.utils.json_to_sheet(diffOnlyExportRows), 'DiffOnly');
      window.XLSX.utils.book_append_sheet(workbook, window.XLSX.utils.json_to_sheet(needReviewExportRows), 'NeedReview');
      ['CONNECTOR', 'TERMINAL', 'CABLE', 'PROTECTION', 'FASTENER'].forEach((sheetName) => {
    ...
    triggerDownload(`${filenameBase}.json`, JSON.stringify({
      summary: summaryRows,
      allDiff: allExportRows,
      diffOnly: diffOnlyExportRows,
      needReview: needReviewExportRows,
  ...
  function renderWorkbenchSummaryCards(diffRows) {
    const summary = summarizeDocumentStatusRows(diffRows);
    const cards = [
      { label: '\u603b\u884c\u6570', value: summary.total },
      { label: resolveStatusMeta('SAME').label, value: summary.counts.SAME || 0 },
      { label: resolveStatusMeta('ADDED').label, value: summary.counts.ADDED || 0 },
      { label: resolveStatusMeta('REMOVED').label, value: summary.counts.REMOVED || 0 },
      { label: resolveStatusMeta('SUBSTITUTED').label, value: summary.counts.SUBSTITUTED || 0 },
      { label: resolveStatusMeta('QTY_CHANGED').label, value: summary.counts.QTY_CHANGED || 0 },
      { label: resolveStatusMeta('NEED_REVIEW').label, value: summary.needReviewTotal || 0 },
    ];
    ['WIRE_SIZE_CHANGED', 'POLE_COUNT_CHANGED', 'LENGTH_CHANGED'].forEach((statusKey) => {
      if (summary.counts[statusKey]) {
        cards.push({ label: resolveStatusMeta(statusKey).label, value: summary.counts[statusKey] || 0 });
      }
    });
    if (summary.counts.ASSEMBLY_TO_PARTS) {
      cards.push({ label: resolveStatusMeta('ASSEMBLY_TO_PARTS').label, value: summary.counts.ASSEMBLY_TO_PARTS || 0 });
    }
    return `
      <div class="bom-workbench-summary-cards">
        ${cards.map((card) => `<article class="bom-workbench-summary-card"><span>${escapeHtml(card.label)}</span><strong>${card.value}</strong></article>`).join('')}
      </div>
    `;
  }
  ...
  function renderReviewWorkbenchView(harnessId) {
    ...
    if (!reviewRows.length) {
      reviewTableMount.innerHTML = '<div class="bom-workbench-empty">\u5f53\u524d\u7ebf\u675f\u6682\u65e0\u9700\u590d\u6838\u5dee\u5f02\u3002</div>';
      return;
    }
    reviewTableMount.innerHTML = `
      <div class="bom-review-head">
        <div>
          <strong>\u4eba\u5de5\u590d\u6838\u8868</strong>
          <span>\u53ea\u4fdd\u7559\u6587\u6863\u53e3\u5f84\u4e0b\u7684\u201c\u9700\u590d\u6838\u201d\u884c\uff0c\u590d\u6838\u5b57\u6bb5\u4f1a\u5199\u5165\u672c\u5730\u6d4f\u89c8\u5668\u5b58\u50a8\u3002</span>
        </div>
      </div>
      <div class="bom-workbench-table-wrap">
        <table class="bom-workbench-table bom-review-table">
          ...
          <tbody>
            ${reviewRows.map((row) => `
              <tr>
                <td>${escapeHtml(row.statusLabel || resolveStatusMeta(row.statusKey || documentStatusFromDiffRow(row)).label)}</td>
  ...
  function renderDiffWorkbenchView(harnessId) {
    ...
    const filterOptions = collectDiffFilterOptions(allDiffRows.filter((row) => row.statusKey !== 'SAME'));
    const columns = [
      ...
      <div class="bom-diff-layout">
        ...
        <aside class="bom-diff-filter-card">
          <div class="bom-diff-filter-head">
            <div>
              <strong>筛选</strong>
              <span>${diffRows.length} / ${allDiffRows.filter((row) => row.statusKey !== 'SAME').length} 条</span>
            </div>
  ...
  function renderSemanticWorkbenchHarness(harnessId) {
    ...
    const harnessSummary = harnessStatusSummaryFor(harnessId);
    const summaryItems = [
      ...
      `差异行 <strong>${diffRows.filter((row) => row.statusKey !== 'SAME').length}</strong>`,
      `高风险 <strong>${diffRows.filter((row) => row.riskLevel === 'High').length}</strong>`,
      ...buildDocumentStatusStatPills(harnessSummary, comparison.summary.assemblyPartCount || 0),
    ];
  ...
  function legacyLaneStatusForRow(row, version) {
  ...
  function laneStatusForRow(harnessId, groupKey, row, rowIndex, version) {
    const item = row.versions?.[version];
    if (!item) return '';
    if (row.rowType === 'assembly_bundle' && version === baseVersion) {
      return 'assembly_bundle';
    }
    const diffRow = diffRowForGroupRow(harnessId, groupKey, rowIndex);
    if (diffRow?.statusKey) {
      return diffRow.statusKey;
    }
    return legacyLaneStatusForRow(row, version);
  }
    ...
    rowBlueprints.forEach((row, index) => {
      versionOrder.forEach((version) => {
        ...
        if (row.directKeys[version]) {
          statuses[version][row.directKeys[version]] = laneStatusForRow(harnessId, group.key, row, index, version);
        }
  ...
  function renderCard(item, row, version, statusKey, groupKey, rowIndex, locked, options = {}) {
    ...
    const status = resolveStatusMeta(statusKey, version);
  ...
  function buildDocumentStatusStatPills(summary, assemblyPartCount = 0) {
    const statusSummary = summary || createDocumentStatusSummary();
    const pills = [
      `${resolveStatusMeta('SAME').label} <strong>${statusSummary.counts.SAME || 0}</strong>`,
      `${resolveStatusMeta('ADDED').label} <strong>${statusSummary.counts.ADDED || 0}</strong>`,
      `${resolveStatusMeta('REMOVED').label} <strong>${statusSummary.counts.REMOVED || 0}</strong>`,
      `${resolveStatusMeta('SUBSTITUTED').label} <strong>${statusSummary.counts.SUBSTITUTED || 0}</strong>`,
    ];
    ['QTY_CHANGED', 'WIRE_SIZE_CHANGED', 'POLE_COUNT_CHANGED', 'LENGTH_CHANGED'].forEach((statusKey) => {
      if (statusSummary.counts[statusKey]) {
        pills.push(`${resolveStatusMeta(statusKey).label} <strong>${statusSummary.counts[statusKey] || 0}</strong>`);
      }
    });
    pills.push(`${resolveStatusMeta('NEED_REVIEW').label} <strong>${statusSummary.needReviewTotal || 0}</strong>`);
    if ((statusSummary.counts.ASSEMBLY_TO_PARTS || 0) || assemblyPartCount) {
      pills.push(`${resolveStatusMeta('ASSEMBLY_TO_PARTS').label} <strong>${statusSummary.counts.ASSEMBLY_TO_PARTS || 0}</strong>`);
      pills.push(`${resolveStatusMeta('assembly_part').label} <strong>${assemblyPartCount || 0}</strong>`);
    }
    return pills;
  }
  ...
  function renderGroup(harnessId, group) {
    const model = buildGroupModel(harnessId, group);
    const groupSummary = groupStatusSummaryFor(harnessId, group.key);
    const statPills = [
      ...versionOrder.map((version) => `${shortLabel(version)} <strong>${model.itemCounts?.[version] || 0}</strong>`),
      ...buildDocumentStatusStatPills(groupSummary, model.assemblyPartCount || 0),
    ];
