...
const DEFAULT_STATE = { bom: 'freeze', connector: 'batch', labor: 'base', equipment: 'base', packaging: 'base', sales: 'quote', mix: 'quote', vave: 'none' };
...
function protocolRowInitialQuote(row, connectorItemMap) {
  const item = connectorItemMap.get(row?.portfolioId);
  if (!item) return null;
  const rows = protocolRowsForItem(row.portfolioId);
  const totalWeight = rows.reduce((sum, itemRow) => sum + protocolRowWeight(itemRow), 0) || rows.length || 1;
  const baseCost = Number(item.baseCost);
  if (!Number.isFinite(baseCost)) return null;
  return baseCost * protocolRowWeight(row) / totalWeight;
}
...
function renderConnectorProtocolOverview() {
  ...
  const note = '初始化规则：同一聚合连接器下全部映射项已达成时切协议价；若已有部分达成则切进度价；若仍全部未达成，则先按样品价。下方明细按 Excel 平铺展示，初始报价在报价 BOM 未提供缓存单价时先按当前基线分摊映射。';
...
function renderTags(m){
  const tags = [`BOM ${m.bom.label}`,`销量 ${m.sales.label}`,`配置 ${m.mix.label}`,`连接器默认 ${m.conn.label}`,`工时 ${m.labor.label}`,`设备 ${m.equip.label}`,`包装 ${m.pack.label}`,`VAVE ${m.vave.label}`];
  if (m.connectorSummary?.overrideCount) {
    tags.splice(4, 0, `连接器覆盖 ${m.connectorSummary.overrideCount} 项`);
  }
...
function renderConnectorPricing(m){
  ...
  const connectorItemMap = new Map((m.connectorItems || []).map((item) => [item.id, item]));
  el.connectorPriceTable.innerHTML = protocolRows.map((row) => {
    ...
    const initialQuote = fmtMaybeMoney(protocolRowInitialQuote(row, connectorItemMap));
    ...
    return `<tr>
      ...
      <td class="connector-number-cell">${initialQuote}</td>
...
function renderArchitecture(m){
  ...
  el.layerDataCount.textContent = '9 个文件';
  el.layerDataMeta.textContent = ['g281_data_master.json', 'g281_data_bom_changes.json', 'g281_data_bom_validation.json', 'g281_data_capital_validation.json', 'g281_data_labor_validation.json', 'g281_data_packaging_validation.json', 'g281_data_connector_protocol_status.json', 'g281_data_history.json', 'g281_data_approvals.json'].join(' / ');
...
function renderEventTable(m){
  ...
  const rows=[['BOM',m.bom.label,`${((m.material/BASE.baseMaterial-1)*100).toFixed(1)}% 材料变动`],['连接器',m.conn.label,connectorImpact],['工时',m.labor.label,`${((m.labor.factor-1)*100).toFixed(1)}% 费率/工时`],['设备',m.equip.label,`${((m.equip.factor-1)*100).toFixed(1)}% 设备分摊`],['包装物流',m.pack.label,`${((m.pack.factor-1)*100).toFixed(1)}% 包装费率`],['销量预测',m.sales.label,`${fmtInt(m.totalVolume)} 套生命周期`],['配置比例',m.mix.label,`${m.currentMix.map(v=>v.toFixed(1)+'%').join(' / ')}`],['VAVE',m.vave.label,`${fmtMoney(m.vave.savings)} 元/套`]];
...
function bind(){
  ...
  el.exportLayerBtn.addEventListener('click',()=>{repo.exportSnapshot(`g281_layer_snapshot_${new Date().toISOString().replace(/[:.]/g,'-')}.json`,{master:BASE,bomChanges:BOM_CHANGE_ROWS,bomValidation:RUNTIME.bomValidation||null,capitalValidation:RUNTIME.capitalValidation||null,laborValidation:RUNTIME.laborValidation||null,packagingValidation:RUNTIME.packagingValidation||null,connectorProtocolStatus:RUNTIME.connectorProtocolStatus||null,history:repo.getHistory(),approvals:repo.getApprovals()})});
  if (el.initConnectorProtocolBtn) {
