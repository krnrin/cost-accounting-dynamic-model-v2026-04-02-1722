...
function protocolTooltip(summary) {
  ...
  return rows.map((row) => `${row.sequence} ${row.groupLabel} | ${row.partNumber} | ${(protocolStatusConfig[row.statusKey]?.label || row.statusLabel || row.statusKey || '')}`).join('\n');
}
...
function renderProtocolDetailRows(itemId) {
  ...
  if (!rows.length) {
    return '<div class="connector-detail-empty">未映射协议明细</div>';
  }
  return `<div class="connector-detail-list">${rows.map((row) => `
    <div class="connector-detail-item">
      <span class="connector-detail-name">${escapeHtml(row.groupLabel || '')}</span>
      <span class="connector-detail-part">${escapeHtml(row.partNumber || '')}</span>
...
function renderConnectorProtocolOverview() {
  ...
  const note = '初始化规则：同一聚合连接器下全部映射项已达成时切协议价；若已有部分达成则切进度价；若仍全部未达成，则先按样品价，后续仍可逐项人工改写。';
