...
function buildCurrentHarnessAllocationRows(model = null) {
  if (!window.G281HarnessProfit?.buildHarnessProfitBreakdown) return [];
  try {
    const breakdown = window.G281HarnessProfit.buildHarnessProfitBreakdown(RUNTIME, model || calcModel());
    return Array.isArray(breakdown?.harnesses) ? breakdown.harnesses : [];
  } catch (error) {
    console.error('Failed to build harness rows for template sheet', error);
    return [];
  }
}
...
function buildLaborVersionTemplateSheetSpec(context) {
  const harnessRows = buildCurrentHarnessAllocationRows();
  ...
  const detailRows = harnessRows.length
    ? harnessRows.map((row) => ([
      ...
      Number(row.counts?.selectedItemCount) || 0,
      Number(row.counts?.wireLineCount) || 0,
      Number(row.unitDirectLaborCost) || 0,
      Number(row.unitManufacturingCost) || 0,
      (Number(row.unitDirectLaborCost) || 0) + (Number(row.unitManufacturingCost) || 0),
      row.allocationBasis?.nonMaterial || '',
    ]))
    : [['', '当前 BOM 暂无线束展开', '', '', '', '', '', '', '']];
  const matrix = [
    ['NO.', 'Function / 分类', 'Part Number / 模板编码', 'Part Name / 项目名称', 'SPEC / 录入说明', 'Quantity / 数值', 'Unit / 单位', 'Remark', 'Other-Remark / 维护提示'],
    ...
    ['', '', '', '', '', '', '', '', ''],
    ['', '线束展开', '', '', '', '', '', '', ''],
    ['', '线束号', '线束名称', 'BOM选中项', '导线条数', '直接人工/套', '制造成本/套', '工时成本合计/套', '分摊口径'],
    ...detailRows,
  ];
  ...
  return {
    ...
    sheetName: context.activeLabel || '模板',
    matrix,
    columnWidths: [56, 120, 180, 150, 110, 118, 118, 128, 260],
    ...
    frozenRows: 1,
    styles: [
      { range: `A1:I${lastRow}`, border: { type: 'ALL', style: 'THIN', color: '#dbe4ee' } },
      { range: 'A1:I1', background: '#e3eaf2', fontWeight: 'bold', wrap: true, horizontalAlignment: 'center', verticalAlignment: 'middle' },
      ...
      { range: `A${sectionHeaderRow}:I${sectionHeaderRow}`, background: '#d9efe6', fontWeight: 'bold', horizontalAlignment: 'left', verticalAlignment: 'middle' },
      { range: `A${detailHeaderRow}:I${detailHeaderRow}`, background: '#eef5fb', fontWeight: 'bold', horizontalAlignment: 'center', verticalAlignment: 'middle', wrap: true },
      { range: `F${detailHeaderRow + 1}:H${lastRow}`, horizontalAlignment: 'right', verticalAlignment: 'middle' },
      { range: `I${detailHeaderRow + 1}:I${lastRow}`, wrap: true, verticalAlignment: 'top' },
    ],
    activeRange: context.fields[0]?.address || 'F2',
  };
}
...
function buildPackagingVersionTemplateSheetSpec(context) {
  const harnessRows = buildCurrentHarnessAllocationRows();
  const totalPackaging = harnessRows.reduce((sum, row) => sum + (Number(row.unitPackagingCost) || 0), 0);
  const baseRows = context.fields.map((field, index) => ([
    index + 1,
  ...
  const detailRows = harnessRows.length
    ? harnessRows.map((row) => ([
      ...
      Number(row.counts?.selectedItemCount) || 0,
      Number(row.counts?.wireLineCount) || 0,
      Number(row.unitPackagingCost) || 0,
      totalPackaging > 0 ? fmtPct((Number(row.unitPackagingCost) || 0) / totalPackaging) : '0.00%',
      getActiveBomAutoExtract()?.sourceLabel || '当前 BOM',
      row.allocationBasis?.nonMaterial || '',
    ]))
    : [['', '当前 BOM 暂无线束展开', '', '', '', '', '', '', '']];
  const matrix = [
    ...
    ['', '', '', '', '', '', '', '', ''],
    ['', '线束展开', '', '', '', '', '', '', ''],
    ['', '线束号', '线束名称', 'BOM选中项', '导线条数', '包装物流/套', '包装占比', '来源', '分摊口径'],
    ...detailRows,
  ];
  const lastRow = matrix.length;
  return {
    ...
    sheetName: context.activeLabel || '模板',
    matrix,
    columnWidths: [56, 120, 180, 150, 110, 118, 100, 160, 250],
    rowHeights: Array.from({ length: lastRow }, (_, index) => {
      const row = index + 1;
      if (row === 1) return 40;
      if (row === sectionHeaderRow || row === detailHeaderRow) return 34;
    ...
    styles: [
      ...
      { range: `A${sectionHeaderRow}:I${sectionHeaderRow}`, background: '#d9efe6', fontWeight: 'bold', horizontalAlignment: 'left', verticalAlignment: 'middle' },
      { range: `A${detailHeaderRow}:I${detailHeaderRow}`, background: '#eef5fb', fontWeight: 'bold', horizontalAlignment: 'center', verticalAlignment: 'middle', wrap: true },
      { range: `F${detailHeaderRow + 1}:G${lastRow}`, horizontalAlignment: 'right', verticalAlignment: 'middle' },
      { range: `H${detailHeaderRow + 1}:I${lastRow}`, wrap: true, verticalAlignment: 'top' },
    ],
    activeRange: context.fields[0]?.address || 'F2',
  };
}
...
function buildVersionTemplateSheetSpec(context) {
  if (context.workbookSnapshot) {
    return {
      ...
      activeRange: context.skipFieldOverlay ? 'A1' : (context.fields[0]?.address || 'F2'),
    };
  }
