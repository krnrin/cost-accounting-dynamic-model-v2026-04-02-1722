...
function timelineColumnCount(eventCount, mountWidth) {
  ...
  if (mountWidth >= 1500) return Math.min(8, eventCount);
  if (mountWidth >= 1220) return Math.min(7, eventCount);
  if (mountWidth >= 880) return Math.min(6, eventCount);
  if (mountWidth >= 640) return Math.min(5, eventCount);
  return Math.min(4, eventCount);
}
...
function renderVersionTimeline() {
  ...
  const rowHeight = columns >= 6 ? 74 : 80;
  const trackTop = 18;
  const cardTop = 30;
  const canvasHeight = Math.ceil(events.length / columns) * rowHeight + 12;
  ...
  el.versionTimelineMount.innerHTML = `
    <section class="timeline-snake" aria-label="\u7248\u672c\u53d1\u5e03\u65f6\u95f4\u7ebf">
      <div class="timeline-snake-head">
        <span class="timeline-stat">${positionedEvents.length} \u4e2a\u4e8b\u4ef6</span>
        <span class="timeline-stat">\u66f4\u65b0\u81f3 ${escapeHtml(formatTimelineDay(latestEvent.timestamp))}</span>
      </div>
      <div class="timeline-canvas" style="height:${canvasHeight}px">
        ...
        ${positionedEvents.map((event) => `
          ...
          <article class="timeline-event${event.active ? ' is-active' : ''}" style="left:${event.left}%; top:${event.cardTop}px" title="${escapeHtml(event.note || `${event.groupLabel} / ${event.label}`)}">
            <span class="timeline-event-group">${escapeHtml(shortTimelineGroupLabel(event.group, event.groupLabel))}</span>
            <strong>${escapeHtml(compactTimelineEventLabel(event))}</strong>
            <span class="timeline-event-time">${escapeHtml(formatTimelineDay(event.timestamp))}</span>
          </article>
        `).join('')}
      </div>
    </section>
