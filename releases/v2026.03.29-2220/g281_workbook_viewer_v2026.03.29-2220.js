(function (global) {
  ...
  function styleToCss(style) {
    ...
    return {
      ...
      background: colorToCss(fill.fgColor, 'transparent'),
  ...
  function ensureToolbarButtons() {
    ...
    const definitions = [
      { id: 'openBomWorkbookViewerBtn', label: '查看 BOM 整表', datasetKey: 'bom' },
  ...
  function openViewer(datasetKey) {
    viewerState.datasetKey = datasets[datasetKey]?.versions ? datasetKey : availableDatasetKeys()[0] || 'bom';
    viewerState.versionKey = resolveDefaultVersionKey(viewerState.datasetKey);
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (openButton) {
        openViewer(openButton.dataset.viewerDataset);
        return;
      }
