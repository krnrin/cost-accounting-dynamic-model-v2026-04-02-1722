(function (global) {
  ...
  function resolveLifecycleStageCandidate(stageKeyCandidate, options, fallback) {
  ...
  function resolveStageMeta(runtimeInput, stageKey, options) {
    ...
    const candidate = resolveLifecycleStageCandidate(stageKey || (options && options.stageKey) || (options && options.baselineKey), options, 'quotation');
    const requested = resolveStageKey(candidate, config, 'quotation');
    return {
      stageKey: requested,
      requestedStageKey: requested,
      lifecycleStageKey: candidate,
    ...
    const totalMetrics = buildMetricEntries(primarySurface.totals, compareSurface.totals, TOTAL_KEYS);
      return {
        stageMeta: {
          ...
          lifecycleStageKey: stageMeta.lifecycleStageKey,
  ...
  function build(runtimeInput, stageKey, options) {
    ...
    const primaryStageOptions = Object.assign({}, safeOptions, { lifecycleStageKey: primaryKey });
    const comparisonStageOptions = Object.assign({}, safeOptions, { lifecycleStageKey: comparisonKey });
    const primaryStage = global.G281HarnessStageCostBuilder
      ? global.G281HarnessStageCostBuilder.build(runtime, primaryKey, primaryStageOptions)
      : { rows: [] };
    const comparisonStage = global.G281HarnessStageCostBuilder
      ? global.G281HarnessStageCostBuilder.build(runtime, comparisonKey, comparisonStageOptions)
      : { rows: [] };
    ...
    const primaryRollup = matchesRollup(safeOptions.projectRollup, primaryKey)
      ...
      : (global.G281ProjectRollupBuilder ? global.G281ProjectRollupBuilder.build(primaryStage.rows || [], runtime, {
        baselineKey: primaryKey,
        lifecycleStageKey: primaryKey,
      }) : { perSet: {} });
    const comparisonRollup = matchesRollup(safeOptions.compareProjectRollup, comparisonKey)
      ? safeOptions.compareProjectRollup
      : (global.G281ProjectRollupBuilder ? global.G281ProjectRollupBuilder.build(comparisonStage.rows || [], runtime, {
        baselineKey: comparisonKey,
        lifecycleStageKey: comparisonKey,
      }) : { perSet: {} });
