...
;(function (global) {
  ...
  function normalizeLifecycleStage(stage, index) {
    ...
    return {
      ...
      aliases: uniqueText(
        []
          .concat(stage && stage.aliases)
          .concat([stage && stage.id, stage && stage.key, stage && stage.stageKey])
      ),
    };
  }

  function normalizeLifecycleStages(stages) {
    return ensureArray(stages).map(normalizeLifecycleStage).filter(Boolean).sort(function (left, right) {
  ...
  function normalizeWorkflowNodes(workflowNodes, config) {
    return ensureArray(workflowNodes).map(function (node, index) {
      ...
      var thresholds = ensureObject(node && node.thresholds);
      return {
        id: nodeId,
  ...
  function normalizeCustomerTemplates(templates, config) {
    return ensureArray(templates).map(function (template, index) {
      ...
      return {
        ...
        exportSheets: ensureArray(template && template.exportSheets).map(function (sheet, index) {
          return {
            id: toText(sheet && sheet.id, 'export-' + (index + 1)),
            sheetRole: toText(sheet && sheet.sheetRole, ''),
            fileName: toText(sheet && sheet.fileName, ''),
            format: toText(sheet && sheet.format, 'xlsx'),
          };
        }).filter(function (entry) {
          return entry.sheetRole || entry.fileName;
        }),
      };
    });
  }
  ...
  function normalizeFinancialRowSpec(spec, index) {
    var rowNumber = Number(spec && spec.rowNumber) || 0;
    return {
      key: toText(spec && spec.key, 'row-' + (index + 1)),
      rowNumber: rowNumber,
      label: toText(spec && spec.label, 'Row ' + (rowNumber || index + 1)),
      category: toText(spec && spec.category, 'metric'),
      detailKind: toText(spec && spec.detailKind, ''),
    };
  }

  function normalizeFinancialRowSpecs(rowSpecs) {
    var entries = ensureArray(rowSpecs);
    return entries.map(normalizeFinancialRowSpec);
  }

  function normalizeFinancialKeyCell(cell, index) {
    return {
      address: toText(cell && cell.address, ''),
      key: toText(cell && cell.key, 'cell-' + (index + 1)),
      label: toText(cell && cell.label, 'Key cell ' + (index + 1)),
    };
  }

  function normalizeFinancialKeyCells(cells) {
    var entries = ensureArray(cells);
    return entries.map(normalizeFinancialKeyCell);
  }

  function normalizeDetailBinding(binding, index) {
    return {
      id: toText(binding && binding.id, 'detail-' + (index + 1)),
      label: toText(binding && binding.label, binding && binding.id),
      sheetRole: toText(binding && binding.sheetRole, ''),
      matchStrategy: toText(binding && binding.matchStrategy, 'keyword'),
      matchValue: toText(binding && (binding.matchValue || binding.sheetName), ''),
      previewRows: Number(binding && binding.previewRows) || 6,
      previewColumns: Number(binding && binding.previewColumns) || 6,
    };
  }

  function normalizeDetailBindings(bindings) {
    var entries = ensureArray(bindings);
    return entries.map(normalizeDetailBinding);
  }

  function normalizeFinancialWorkbook(workbook) {
    var safeWorkbook = ensureObject(workbook);
    var rowSpecs = normalizeFinancialRowSpecs(safeWorkbook.rowSpecs);
    var keyCells = normalizeFinancialKeyCells(safeWorkbook.keyCellSpecs || safeWorkbook.keyCells);
    var detailBindings = normalizeDetailBindings(safeWorkbook.detailSheetBindings);
    return {
      summarySheetAliases: ensureArray(safeWorkbook.summarySheetAliases).length
        ? ensureArray(safeWorkbook.summarySheetAliases)
        : DEFAULT_FINANCIAL_WORKBOOK.summarySheetAliases,
  ...
  function isLikelyConfigObject(value) {
    return Boolean(
      value
      && typeof value === 'object'
      && !Array.isArray(value)
  ...
  function isLikelyRuntimeObject(value) {
    return Boolean(
      value
      && typeof value === 'object'
      && !Array.isArray(value)
      && !isLikelyConfigObject(value)
    );
  }

  function resolveRouterProjectId() {
    if (!global.G281PageRouter || typeof global.G281PageRouter.resolveState !== 'function') {
      return '';
    }
  ...
  function loadFromRegistry(projectId) {
    ...
    if (requestedId && typeof registry.getProject === 'function') {
      registryConfig = registry.getProject(requestedId);
    }
    if (!registryConfig && typeof registry.getActiveConfig === 'function') {
      registryConfig = registry.getActiveConfig();
    }
  ...
  function normalizeLoadProjectOptions(pathOrRuntime, maybeOptions) {
    ...
    var looksLikeOptionsObject = Boolean(
      pathOrRuntime
      && typeof pathOrRuntime === 'object'
      && !Array.isArray(pathOrRuntime)
      && (
        pathOrRuntime.defaultConfigPath != null
        || pathOrRuntime.configPath != null
        || pathOrRuntime.requireActiveProject != null
        || pathOrRuntime.runtime != null
        || pathOrRuntime.fallbackPaths != null
      )
    );

    if (typeof pathOrRuntime === 'string') {
      defaultConfigPath = pathOrRuntime;
    } else if (!looksLikeOptionsObject && isLikelyConfigObject(pathOrRuntime)) {
      configObject = pathOrRuntime;
    } else if (isLikelyRuntimeObject(pathOrRuntime)) {
      runtime = pathOrRuntime;
    } else if (pathOrRuntime && typeof pathOrRuntime === 'object') {
      options = Object.assign({}, pathOrRuntime, maybeOptions);
      if (isLikelyConfigObject(options.config)) {
        configObject = options.config;
      }
      if (isLikelyRuntimeObject(options.runtime)) {
    ...
    return {
      ...
      fallbackPaths: uniqueText(
        []
          .concat(options.fallbackPaths)
          .concat([defaultConfigPath || options.defaultConfigPath || options.configPath, DEFAULT_PROJECT_CONFIG_PATH, '../config/g281.project.json'])
      ),
    };
  }
  ...
  var ConfigLoader = {
    ...
    loadProjectConfig: async function (pathOrRuntime, maybeOptions) {
      ...
      if (options.config) {
        return ConfigLoader.load(options.config).config;
      }

      var requestedProjectId = options.projectId || resolveRouterProjectId();
      if (requestedProjectId) {
        var requestedRegistryConfig = loadFromRegistry(requestedProjectId);
        if (requestedRegistryConfig) return requestedRegistryConfig;

        var requestedStorage = ConfigLoader.loadFromStorage(requestedProjectId);
        if (requestedStorage && requestedStorage.config) {
          return requestedStorage.config;
        }

        if (options.requireActiveProject) {
          throw new Error('Active project not found: ' + requestedProjectId);
        }
      }
      ...
      if (options.requireActiveProject) {
        throw new Error('Active project is required but not available');
      }
      ...
      for (var index = 0; index < options.fallbackPaths.length; index += 1) {
        ...
        try {
          return await loadFromPath(candidatePath);
        } catch (error) {
