(function () {
  ...
  function buildAmountDelta(quoteItem, fixedItem, amountLabel) {
    ...
    if (Math.abs(diff) < 1e-9 && Number.isFinite(quoteQty) && Number.isFinite(fixedQty) && Math.abs(fixedQty - quoteQty) < 1e-9) {
      return createDelta(
        '一致',
        'delta-same',
        `报价 ${fmtMoney(quoteAmount)} 元，定点 ${fmtMoney(fixedAmount)} 元；数量 ${fmtQty(quoteQty)} ${unit}`.trim(),
      );
    }
  ...
  function buildGroupMeta(group, comparison) {
    ...
    return `${amountLabel}: 报价 ${fmtMoney(group.quoteAmount)} 元 / 定点 ${fmtMoney(group.fixedAmount)} 元 / 差异 ${signedMoney(deltaAmount)}`;
  }
