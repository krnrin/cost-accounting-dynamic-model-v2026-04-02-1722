  ...
  const drivers = [
    {
      ...
      meta: `${m.bom.label} · 对基准 ${fmtSignedMoney((Number(m.material) || 0) - baseMaterial)} 元/套`,
...
function renderArchitecture(m){
  ...
  el.layerDataCount.textContent = '10 个文件';
...
function render(m){
  el.scenarioName.value=m.d.scenarioName; renderTags(m); renderSummary(m); renderKPIs(m); renderProfitDrivers(m); renderBomAnalysis(m); renderConnectorPricing(m); renderArchitecture(m); renderCostBridge(m); renderAnnualChart(m); renderConfigBars(m); renderEventTable(m); renderCompare(m); renderCapital(m); renderAnnualTable(m); clearDirty();
}
