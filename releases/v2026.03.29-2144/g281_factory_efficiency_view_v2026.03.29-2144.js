(function (global) {
  ...
  function renderPanel(panelState, tabKey, sectionData) {
    ...
    panelState.remark.textContent = sectionData.note || '当前页签未设置备注。';
  }

  function renderPanels() {
    TAB_CONFIG.forEach((tab) => {
      renderPanel(state.panels[tab.key], tab.key, state.data[tab.key]);
    });
    updateStatus();
  ...
  function mergeIncomingData(data) {
    ...
    state.baseData = incoming;
    if (state.editMode && state.dirty) {
      updateStatus();
      return;
    }
    const overrides = loadOverrides();
    state.data = applyOverrides(incoming, overrides);
