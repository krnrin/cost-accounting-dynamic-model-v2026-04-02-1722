...
function ensureDashboardUiScaffold() {
  ...
  const scenarioPanel = sidebarPanels[0] || null;
  const versionPanel = sidebarPanels[1] || null;
  ...
  sidebarPanels.slice(2).forEach((panel) => panel.classList.add('legacy-hidden-panel'));

  if (main) {
    let timelineStrip = main.querySelector('.timeline-strip');
    if (!timelineStrip) {
    ...
    let managementGrid = main.querySelector('.management-top-grid');
    if (!managementGrid) {
...
function renderVersionTimeline() {
  ...
  if (!timestamps.length) {
    el.versionTimelineMount.innerHTML = '<div class="timeline-empty">鏆傛棤鐗堟湰鏃堕棿绾挎暟鎹€?/div>';
    return;
  }
  ...
  el.versionTimelineMount.innerHTML = rows.map((row) => `
    <section class="timeline-group">
      <div class="timeline-group-label">
        ...
        <span>褰撳墠锛?${escapeHtml(row.activeLabel)}</span>
      </div>
...
function renderDataManagementOverview(m) {
  ...
  const cards = [
    ...
    {
      title: '宸ュ巶鏁堢巼 / 杩愯惀璐圭巼',
      state: Number.isFinite(Number(m?.operating)) ? `${fmtMoney(m.operating)} 鍏?濂?` : '鍙紪杈? / 鍙姣?',
      meta: `褰撳墠宸ユ椂 ${versionOptionLabel('labor', state.labor)} / 璁惧 ${versionOptionLabel('equipment', state.equipment)}`,
    },
  ];
