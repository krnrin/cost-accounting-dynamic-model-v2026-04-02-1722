...
;(function (root) {
  ...
  const STORAGE_NAMESPACE = 'lifecycle_cost_platform';
  const DEFAULT_STORAGE_PREFIX = 'lifecycle';
  const STORAGE_KEY = `${STORAGE_NAMESPACE}.project_registry.v1`;
  const ACTIVE_PROJECT_KEY = `${STORAGE_NAMESPACE}.active_project.v1`;
  const LEGACY_STORAGE_KEY = 'g281_project_registry';
  const LEGACY_ACTIVE_PROJECT_KEY = 'g281_active_project';
  ...
  function normalizeStoragePrefix(value, fallback) {
  ...
  function loadRegistry() {
    ...
    try {
      ...
      loadedFromLegacy = !localStorage.getItem(STORAGE_KEY) && !!localStorage.getItem(LEGACY_STORAGE_KEY);
      registry = raw ? JSON.parse(raw) : {};
    } catch (_) {
      registry = {};
    }
    try {
      activeProjectCode = localStorage.getItem(ACTIVE_PROJECT_KEY)
        || localStorage.getItem(LEGACY_ACTIVE_PROJECT_KEY)
        || null;
    } catch (_) {
      activeProjectCode = null;
    }
    if (loadedFromLegacy && Object.keys(registry).length) {
      saveRegistry();
    }
  }
  ...
  function saveRegistry() {
    try {
      ...
      localStorage.setItem(LEGACY_STORAGE_KEY, JSON.stringify(registry));
      if (activeProjectCode) {
        localStorage.setItem(ACTIVE_PROJECT_KEY, activeProjectCode);
        localStorage.setItem(LEGACY_ACTIVE_PROJECT_KEY, activeProjectCode);
      } else {
  ...
  function normalizeProjectCode(projectCode, config) {
    const requested = String(
      projectCode || (config && (config.projectCode || config.projectId)) || ''
    ).trim().toUpperCase();
    if (!requested) {
      throw new Error('[ProjectRegistry] projectCode is required');
    }
  ...
  function registerNewProject(projectCode, config) {
    ...
    const normalizedConfig = Object.assign({}, clonePlain(config) || {}, {
      projectId: normalizedCode,
      projectCode: normalizedCode,
      storageKeyPrefix: String(
        (config && config.storageKeyPrefix) || normalizedCode
      ).trim().toLowerCase(),
    });

    let finalConfig = normalizedConfig;
    if (root.ConfigLoader && typeof root.ConfigLoader.load === 'function') {
      const loaded = root.ConfigLoader.load(normalizedConfig);
      if (!loaded || !loaded.config) {
        throw new Error('[ProjectRegistry] Failed to normalize project config');
      }
      if (loaded.errors && loaded.errors.length) {
        throw new Error('[ProjectRegistry] ' + loaded.errors.join('; '));
      }
      finalConfig = root.ConfigLoader._clonePlain
        ? root.ConfigLoader._clonePlain(loaded.config, loaded.config)
        : clonePlain(loaded.config);
    }

    if (root.ConfigLoader && typeof root.ConfigLoader.saveToStorage === 'function') {
      root.ConfigLoader.saveToStorage(finalConfig);
    } else {
      localStorage.setItem(normalizedCode + '.projectConfig', JSON.stringify(finalConfig));
    }

    return registerProject(normalizedCode, finalConfig);
  }
  ...
  function getDbName(projectCode) {
    const resolvedProjectCode = projectCode || activeProjectCode || 'default';
    const projectConfig = registry[resolvedProjectCode] || null;
    const storagePrefix = normalizeStoragePrefix(
      projectConfig && projectConfig.storageKeyPrefix,
      DEFAULT_STORAGE_PREFIX
    );
    return `${storagePrefix}-bom-store-${normalizeStoragePrefix(resolvedProjectCode, 'default')}`;
  }
