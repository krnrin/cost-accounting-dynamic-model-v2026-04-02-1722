(function (global) {
  ...
  function mergeSpecs(defaultSpecs, customSpecs, keySelector) {
    const merged = new Map();
    safeArray(defaultSpecs).forEach((spec) => {
      const key = toText(keySelector(spec), '');
      if (!key) return;
      merged.set(key, Object.assign({}, clonePlain(spec, {})));
    });
    safeArray(customSpecs).forEach((spec) => {
      const key = toText(keySelector(spec), '');
      if (!key) return;
      merged.set(key, Object.assign({}, merged.get(key) || {}, clonePlain(spec, {})));
    });
  ...
  function getRowSpecs(projectConfig) {
    ...
    return mergeSpecs(FIXED_ROW_SPECS, specs, (spec) => spec && (spec.key || spec.rowNumber));
  }

  function getKeyCellSpecs(projectConfig) {
    const workbookConfig = getFinancialWorkbookConfig(projectConfig);
    const specs = safeArray(workbookConfig.keyCellSpecs || workbookConfig.keyCells);
    return mergeSpecs(DEFAULT_KEY_CELL_SPECS, specs, (spec) => toText(spec && spec.address, '').toUpperCase());
  }
