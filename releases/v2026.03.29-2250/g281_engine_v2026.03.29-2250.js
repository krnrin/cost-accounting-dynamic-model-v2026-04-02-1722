(function (global) {
  ...
  function resolvePureFinancialVersionKey(currentState, draft) {
    ...
    if ((currentState.connector || '') !== 'batch') return '';
    if (draft && draft.connectorPricing && Object.keys(draft.connectorPricing).length) return '';

    const groups = ['bom', 'metal', 'sales', 'labor', 'equipment', 'packaging', 'mix'];
    let resolved = '';
  ...
  function resolveExactFinancialVersion(runtime, base, currentState, draft, bomSnapshot, connectorScenario) {
    ...
    if ((currentState && currentState.connector) !== 'batch') {
      return null;
    }
  ...
  function computeModel(runtime, draft, state) {
    ...
    const annualDrop = BASE.versions.annualDrop?.[currentState.annualDrop] || { label: currentState.annualDrop, annualRate: 0 };
    const oneTimeCustomer = BASE.versions.oneTimeCustomer?.[currentState.oneTimeCustomer] || { label: currentState.oneTimeCustomer, amountTotal: 0 };
    const rebate = BASE.versions.rebate?.[currentState.rebate] || { label: currentState.rebate, amountPerSet: 0 };
    ...
    const annualDropRate = Math.max(0, Number(annualDrop.annualRate) || 0);
    const oneTimeCustomerAmountTotal = Math.max(0, Number(oneTimeCustomer.amountTotal) || 0);
    const rebateAmountPerSet = Math.max(0, Number(rebate.amountPerSet) || 0);
    const oneTimeCustomerPerSet = lifecycleVolume ? oneTimeCustomerAmountTotal / lifecycleVolume : 0;
    ...
    const exactFinancial = (annualDropRate || oneTimeCustomerAmountTotal || rebateAmountPerSet)
      ? null
    ...
    const annual = BASE.years.map((year, index) => {
      ...
      const annualDropFactor = Math.max(0, 1 - annualDropRate * index);
      const asp = Math.max(0, aspBase * annualDropFactor - rebateAmountPerSet);
      const revenue = volume * asp + volume * oneTimeCustomerPerSet;
      const cost = volume * operating;
      const profit = revenue - cost;
      const margin = revenue ? profit / revenue : 0;
      return { year, volume, asp, revenue, cost, profit, margin };
  ...
  function computeModelV2(runtime, draft, state) {
    ...
    const exactFinancialVersion = financialVersion(runtime, exactFinancialKey);
