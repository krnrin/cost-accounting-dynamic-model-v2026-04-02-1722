...
export default function VersionManager() {
  ...
  const columns = [
    ...
    { title: '到厂价汇总', dataIndex: 'results', render: (r: QuoteSnapshot['results']) => r ? `¥${r.totalDeliveredPrice.toFixed(2)}` : '-' },
    {
      title: '操作',
  ...
  return (
    <div style= maxWidth: 1200, margin: '0 auto', padding: 24 >
      ...
      <Title heading={5} style= marginTop: 24 >报价快照历史</Title>
      <Table columns={columns} dataSource={quoteSnapshots} rowKey="id" pagination= pageSize: 20  />

      <Title heading={5} style= marginTop: 24 >参数变更时间线</Title>
      <Timeline>
        {snapshots.slice(0, 20).map((snap: SettingsSnapshot) => (
          <Timeline.Item key={snap.id} time={new Date(snap.timestamp).toLocaleString('zh-CN')}>
            ...
            <Text style= marginLeft: 8 >{snap.summary}</Text>
          </Timeline.Item>
        ))}
      </Timeline>
          ...
          return (
            <div>
              ...
              <Table
                ...
                size="small"
              />
              <Title heading={6} style= marginTop: 16 >结果差异</Title>
              <Table
