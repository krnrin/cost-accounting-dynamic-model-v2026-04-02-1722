...
import { IconArrowLeft, IconSave, IconUpload } from '@douyinfe/semi-icons';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <Space spacing={4}>
          ...
          <Tag size="small" color="blue">{harnesses.length} 线束</Tag>
          {isDirty && <Tag size="small" color="orange">未保存</Tag>}
        </Space>

        {/* Right: actions */}
        <Space spacing={4}>
          {/* Import BOM popover — select target harness */}
          <RoleGuard field="bomEdit">
            ...
            >
              <Button icon={<IconUpload />} size="small">导入BOM</Button>
            </Popover>
          </RoleGuard>
          ...
          {isDirty && (
            ...
            >
              <Button theme="light" type="danger" size="small">放弃更改</Button>
            </Popconfirm>
          )}
      ...
      <div style={{
        ...
        <Space spacing="medium">
          <Text type="secondary">
            线束: <Text strong>{totals.count}</Text>/{harnesses.length}
          </Text>
          <Divider layout="vertical" />
          <Text>材料: <Text strong style={{ color: 'var(--semi-color-primary)' }}>¥{totals.material.toFixed(2)}</Text></Text>
          <Text>人工: <Text strong>¥{totals.labor.toFixed(2)}</Text></Text>
          <Text>制造: <Text strong>¥{totals.mfg.toFixed(2)}</Text></Text>
          <Text>出厂: <Text strong style={{ color: 'var(--semi-color-warning)' }}>¥{totals.exFactory.toFixed(2)}</Text></Text>
          <Text>到厂: <Text strong style={{ color: 'var(--semi-color-danger)' }}>¥{totals.delivered.toFixed(2)}</Text></Text>
        </Space>
      </div>
