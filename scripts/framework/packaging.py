...
class PackagingValidationExtractor(BaseExtractor):
    def get_config(self) -> dict[str, Any]:
        return {
            "name": "packaging",
            "description": "Packaging validation (quote vs fixed)",
            "workbook_pattern": "核算",
            "default_output": "g281_data_packaging_validation.json",
            "sheet_indices": {
                "summary": 0,
                "assessment": 1,
                "packaging": 10,
    ...
    def build_payload(self, quote_workbook: Path, fixed_workbook: Path) -> dict[str, Any]:
        ...
        quote_groups = self.workbook_groups(quote_ctx, "quote")
        fixed_groups = self.workbook_groups(fixed_ctx, "fixed")
        ...
        return {
            ...
            "versionSnapshots": {
                "quote": self.build_snapshot(quote_ctx, "quote"),
                "fixed": self.build_snapshot(fixed_ctx, "fixed"),
            },
            "comparisons": comparisons,
        }

    def build_snapshot(self, ctx: dict[str, Any], kind: str) -> dict[str, Any]:
    ...
    def workbook_groups(self, ctx: dict[str, Any], kind: str) -> dict[str, list[dict[str, Any]]]:
        snapshot = self.build_snapshot(ctx, kind)
        scenario_items = [
            make_item("apply.packInner", "内外包装", "元/套", ctx["sheets"]["packaging"]["title"], "E25+F25", snapshot["packInner"], "", note="程序字段 = 内包装 + 外包装。"),
            make_item("apply.packFreight", "运输费", "元/套", ctx["sheets"]["packaging"]["title"], "G25+H25", snapshot["packFreight"], "", note="程序字段 = 运费 + 超额运费。"),
            make_item("apply.packWarehouse", "仓储费", "元/套", ctx["sheets"]["packaging"]["title"], "J25+K25", snapshot["packWarehouse"], "", note="程序字段 = 三方仓费用 + 仓储费。"),
            make_item("apply.packOther", "短驳 / 其他", "元/套", ctx["sheets"]["packaging"]["title"], "I25", snapshot["packOther"], "", note="程序字段 = 短驳（合肥仓库到客户）+ 其他。"),
            make_item("apply.packTotal", "程序包装物流合计", "元/套", ctx["sheets"]["packaging"]["title"], "L25", snapshot["packTotal"], "", note="四个程序字段汇总后应与包装物流费用页单套合计一致。"),
            self.assessment_item(ctx, "F32", "assessment.packagingPerSet", "项目评估单套包装物流", "元/PCS", "来源：项目评估汇总引用包装物流费用!L25。"),
        ]

        breakdown_items = [
            self.summary_cell_item(ctx, "E25", "breakdown.innerPack", "内包装", "元/套"),
            self.summary_cell_item(ctx, "F25", "breakdown.outerPack", "外包装", "元/套"),
            self.summary_cell_item(ctx, "G25", "breakdown.freight", "运费", "元/套"),
            self.summary_cell_item(ctx, "H25", "breakdown.extraFreight", "超额运费", "元/套"),
            self.summary_cell_item(ctx, "I25", "breakdown.shortHaulOther", "短驳 / 其他", "元/套"),
            self.summary_cell_item(ctx, "J25", "breakdown.thirdWarehouse", "三方仓费用", "元/套"),
            self.summary_cell_item(ctx, "K25", "breakdown.storage", "仓储费", "元/套"),
            self.summary_cell_item(ctx, "L25", "breakdown.total", "包装物流单套合计", "元/套"),
            self.assessment_item(ctx, "F32", "breakdown.assessmentPackaging", "项目评估包装物流引用", "元/PCS", "项目评估汇总 F32 引用包装物流费用!L25。"),
        ]
        ...
        while True:
            ...
            if not marker:
                ...
                if row > 80: break
                continue
            if marker == "单套成本": break
            detail_items.append(self.packaging_row_item(ctx, row))
            row += 1

        return {
            "scenario": [{"key": "scenario.mapping", "label": "程序包装映射", "meta": "把包装物流费用页单套拆分映射到程序当前 4 个包装输入项。", "items": scenario_items}],
            "breakdown": [{"key": "breakdown.perSet", "label": "单套包装拆分", "meta": "来源：包装物流费用 + 项目评估/项目汇总引用。", "items": breakdown_items}],
            "details": [{"key": "details.parts", "label": "零件包装明细", "meta": "来源：包装物流费用逐件明细。", "items": detail_items}],
        }

    def packaging_row_item(self, ctx: dict[str, Any], row: int) -> dict[str, Any]:
        ws_values = ctx["sheets"]["packaging"]["values"]
    ...
    def summary_cell_item(self, ctx: dict[str, Any], cell: str, item_key: str, label: str, unit: str, note: str = "") -> dict[str, Any]:
        ws_values = ctx["sheets"]["packaging"]["values"]
        ws_formulas = ctx["sheets"]["packaging"]["formulas"]
        return make_item(item_key, label, unit, ctx["sheets"]["packaging"]["title"], cell, ws_values[cell].value, ws_formulas[cell].value, note=note)

    def assessment_item(self, ctx: dict[str, Any], cell: str, item_key: str, label: str, unit: str, note: str = "") -> dict[str, Any]:
        ws_values = ctx["sheets"]["assessment"]["values"]
        ws_formulas = ctx["sheets"]["assessment"]["formulas"]
        return make_item(item_key, label, unit, ctx["sheets"]["assessment"]["title"], cell, ws_values[cell].value, ws_formulas[cell].value, note=note)
    ...
    def align_items(self, quote_items: list[dict[str, Any]], fixed_items: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in fixed_items:
            if item["itemKey"] not in quote_map: order.append(item["itemKey"])

        aligned = []
        summary = {"quoteCount": len(quote_items), "fixedCount": len(fixed_items), "matchedCount": 0, "quoteOnlyCount": 0, "fixedOnlyCount": 0, "differenceCount": 0}

        for item_key in order:
            quote = quote_map.get(item_key); fixed = fixed_map.get(item_key)
            if quote and fixed: status = "matched"; summary["matchedCount"] += 1
            elif quote: status = "quote_only"; summary["quoteOnlyCount"] += 1
            else: status = "fixed_only"; summary["fixedOnlyCount"] += 1

            if quote and fixed:
                q_num = quote.get("numericValue"); f_num = fixed.get("numericValue")
                if q_num is not None and f_num is not None:
                    if abs(float(q_num) - float(f_num)) > 1e-6: summary["differenceCount"] += 1
                elif quote.get("value") != fixed.get("value") or quote.get("note") != fixed.get("note"):
                    summary["differenceCount"] += 1
            else: summary["differenceCount"] += 1
            aligned.append({"status": status, "quote": quote, "fixed": fixed})
        return {"aligned": aligned, "summary": summary}
    ...
    def build_scope(self, scope_id: str, scope_label: str, hint: str, quote_groups: list[dict[str, Any]], fixed_groups: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for group in fixed_groups:
            if group["key"] not in order: order.append(group["key"])

        groups = []
        scope_summary = {"quoteCount": 0, "fixedCount": 0, "matchedCount": 0, "quoteOnlyCount": 0, "fixedOnlyCount": 0, "differenceCount": 0}

        for key in order:
            quote_group = next((group for group in quote_groups if group["key"] == key), None)
            fixed_group = fixed_map.get(key)
            aligned = self.align_items(quote_group["items"] if quote_group else [], fixed_group["items"] if fixed_group else [])
            groups.append({
                "key": key,
            ...
            for k in scope_summary: scope_summary[k] += aligned["summary"][k]
        return {"scopeId": scope_id, "scopeLabel": scope_label, "hint": hint, "groups": groups, "summary": scope_summary}
