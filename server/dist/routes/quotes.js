...
router.get('/project/:pid', async (req, res, next) => {
    try {
        const quotes = await QuoteService.getQuotesByProject(req.params.pid);
...
router.get('/scenario/:sid', async (req, res, next) => {
    try {
        const quotes = await QuoteService.getQuotesByScenario(req.params.sid);
        res.json({ data: quotes });
    }
    catch (error) {
        next(error);
...
router.get('/:id/compare', async (req, res, next) => {
    try {
        const quote = await QuoteService.compareQuote(req.params.id);
        res.json({ data: quote });
    }
    catch (error) {
        next(error);
...
router.get('/:id/effective-price', async (req, res, next) => {
    try {
        const quote = await QuoteService.getEffectivePrice(req.params.id);
        res.json({ data: quote });
    }
    catch (error) {
        next(error);
...
router.post('/scenario/:sid', requireRole(['ADMIN', 'MANAGER', 'ENGINEER']), async (req, res, next) => {
    try {
        const validatedData = quoteSchema.parse({ ...req.body, scenarioId: req.params.sid });
...
router.post('/:id/confirm', requireRole(['ADMIN', 'MANAGER', 'ENGINEER']), async (req, res, next) => {
    try {
        const quote = await QuoteService.confirmQuote(req.params.id, req.user?.id);
        await AuditService.log({
            userId: req.user.id,
...
router.put('/:id', requireRole(['ADMIN', 'MANAGER', 'ENGINEER']), async (req, res, next) => {
    try {
        ...
        const quote = await QuoteService.updateQuote(req.params.id, validatedData);
        await AuditService.log({
            userId: req.user.id,
            projectId: quote.projectId,
            action: 'UPDATE',
...
router.delete('/:id', requireRole(['ADMIN', 'MANAGER']), async (req, res, next) => {
    try {
        const quote = await QuoteService.deleteQuote(req.params.id);
        await AuditService.log({
            userId: req.user.id,
            projectId: quote.projectId,
            action: 'DELETE',
