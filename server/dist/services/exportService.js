...
async function loadProjectBundle(projectId) {
    ...
    const costRates = fromJson(project.costRates, {});
    const metalPrices = fromJson(project.metalPrices, {});
    const volumes = fromJson(project.volumes, {});
...
async function loadQuoteBundle(quoteId) {
    ...
    const allocations = quote.scenarioId && quote.harnessId
        ...
        : [];
    const quoteData = fromJson(quote.data, {});
    const quoteParams = fromJson(quote.quoteParams, {});
    const quoteResult = fromJson(quote.quoteResult, {});
    const lockedFields = fromJson(quote.lockedFields, []);
    const editableFields = fromJson(quote.editableFields, []);
...
async function pdfBuffer(render) {
...
export class ExportService {
    static async export(type, payload) {
        const resourceType = payload.quoteId ? 'quote' : 'project';
        ...
        if (!payload.quoteId) {
            const err = new Error('quoteId is required');
            err.status = 400;
            throw err;
        }
