...
router.post('/:sid/import-baseline', requireRole(['ADMIN', 'MANAGER', 'ENGINEER']), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const projectId = req.params.id || req.params.pid;
    const input = importBaselineSchema.parse(req.body);
    const data = await ScenarioImportService.importBaseline(projectId as string, req.params.sid as string, input);
    await AuditService.log({
      userId: req.user!.id,
      projectId: projectId as string,
      action: 'UPDATE',
      entity: 'scenario',
      entityId: req.params.sid as string,
      details: {
        baseline: 'custom',
