 ...
 * 确保用户只能导出自己有权限访问的项目
 */
async function checkProjectMembership(req: Request, res: Response, next: NextFunction) {
  try {
...
async function handleExport(type: 'excel' | 'pdf', req: Request, res: Response, next: NextFunction) {
  try {
    const payload = exportRequestSchema.parse(req.body ?? {});
