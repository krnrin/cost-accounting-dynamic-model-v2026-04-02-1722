...
function getQuoteUnitPrice(quote: any) {
  const quoteResult = fromJson<Record<string, any>>(quote.quoteResult, {});
  return toNumber(
    quote.effectivePrice ?? quote.arrivalPrice ?? quoteResult.arrivalPrice ?? quoteResult.deliveredPrice,
    0,
  );
}
