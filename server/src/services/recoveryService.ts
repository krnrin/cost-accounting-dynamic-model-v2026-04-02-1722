...
export class RecoveryService {
  ...
  static async create(allocationItemId: string, input: any) {
  ...
  static async forecast(allocationItemId: string) {
    const allocation = await prisma.allocationItem.findUnique({ where: { id: allocationItemId } });
    if (!allocation) {
      const err: any = new Error('Allocation not found');
      err.status = 404;
  ...
  static async complete(allocationItemId: string) {
    const allocation = await prisma.allocationItem.findUnique({ where: { id: allocationItemId } });
    if (!allocation) {
      const err: any = new Error('Allocation not found');
      err.status = 404;
